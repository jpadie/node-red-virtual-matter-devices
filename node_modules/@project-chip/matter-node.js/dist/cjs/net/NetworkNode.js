...
var NetworkNode_exports = {};
__export(NetworkNode_exports, {
  NetworkNode: () => NetworkNode
});
module.exports = __toCommonJS(NetworkNode_exports);
var import_log = require("@project-chip/matter.js/log");
var import_net = require("@project-chip/matter.js/net");
var import_util = require("@project-chip/matter.js/util");
var import_os = require("os");
var import_UdpChannelNode = require("./UdpChannelNode.js");
/**
 * @license
 * Copyright 2022-2024 Matter.js Authors
 * SPDX-License-Identifier: Apache-2.0
 */
const logger = import_log.Logger.get("NetworkNode");
class NetworkNode extends import_net.Network {
  static getMulticastInterfaceIpv4(netInterface) {
    const netInterfaceInfo = (0, import_os.networkInterfaces)()[netInterface];
    if (netInterfaceInfo === void 0) throw new import_net.NetworkError(`Unknown interface: ${netInterface}`);
  ...
  static netInterfaces = new import_util.Cache(
    "Network interface",
  ...
  static getNetInterfaceForRemoveAddress(ip) {
    if (ip.includes("%")) {
      return ip.split("%")[1];
    } else {
      ...
      for (const name in interfaces) {
        ...
        for (const { address, netmask } of netInterfaces) {
          if ((0, import_util.onSameNetwork)(ip, address, netmask)) {
            return this.getNetInterfaceZoneIpv6Internal(name, netInterfaces);
          }
        }
      }
      if ((0, import_util.isIPv6)(ip)) {
        if (ip.startsWith("fd")) {
          return "";
        }
      }
  ...
  getNetInterfaces(configuration = []) {
    ...
    for (const name in interfaces) {
      ...
      let type = import_net.InterfaceType.Ethernet;
  ...
  createUdpChannel(options) {
    return import_UdpChannelNode.UdpChannelNode.create(options);
  }
}
