...
var import_context = require("@project-chip/matter.js/behavior/context");
 ...
 */
class NodeJsActionTracer extends import_context.ActionTracer {
  #path;
  #output;
  #write;
  constructor(path) {
  ...
  static configure(env) {
    env.vars.use(() => {
      if (env.has(import_context.ActionTracer)) {
        env.delete(env.get(import_context.ActionTracer).constructor);
      }
      ...
      env.set(import_context.ActionTracer, tracer);
      env.runtime.add(tracer);
    });
  }
  [Symbol.asyncDispose]() {
    import_util.MaybePromise.then(this.#write, () => this.#output?.close());
  }
  record(action) {
    const raw = {
      ...action,
    ...
    this.#write = import_util.MaybePromise.then(this.#write, () => this.#record(raw));
  }
  async #record(action) {
...
function replacer(_key, value) {
  if (typeof value === "bigint" || ArrayBuffer.isView(value)) {
    return (0, import_util.serialize)(value);
  }
  return value;
}
