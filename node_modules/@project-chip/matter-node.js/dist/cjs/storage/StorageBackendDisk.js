...
var import_storage = require("@project-chip/matter.js/storage");
 ...
 */
class StorageBackendDisk extends import_storage.SyncStorage {
  localStorage;
  isInitialized = false;
  ...
  getContextBaseKey(contexts, allowEmptyContext = false) {
    ...
    if (!contextKey.length && !allowEmptyContext || contextKey.includes("..") || contextKey.startsWith(".") || contextKey.endsWith("."))
      throw new import_storage.StorageError("Context must not be an empty and not contain dots.");
    return contextKey;
  }
  buildStorageKey(contexts, key) {
    if (!key.length) {
      throw new import_storage.StorageError("Key must not be an empty string.");
    }
  ...
  get(contexts, key) {
    ...
    return (0, import_storage.fromJson)(value);
  }
  set(contexts, keyOrValues, value) {
    if (typeof keyOrValues === "string") {
      this.localStorage.setItem(this.buildStorageKey(contexts, keyOrValues), (0, import_storage.toJson)(value));
    } else {
      for (const [key, value2] of Object.entries(keyOrValues)) {
        this.localStorage.setItem(this.buildStorageKey(contexts, key), (0, import_storage.toJson)(value2));
      }
    }
  }
  delete(contexts, key) {
