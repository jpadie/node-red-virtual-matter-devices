...
class GeneralCommissioningServer extends GeneralCommissioningBehavior {
  ...
  async armFailSafe({ breadcrumb, expiryLengthSeconds }) {
    ...
    try {
      if (!device.isFailsafeArmed() && this.agent.get(AdministratorCommissioningServer).state.windowStatus !== AdministratorCommissioning.CommissioningWindowStatus.WindowNotOpen && !this.session.isPase) {
