...
class Logger {
  ...
  static set level(level) {
    ...
    if (typeof level === "string") {
      if (level.match(/^[0-9]+$/)) {
        ...
        levelNum = Level[level.toUpperCase()];
        if (levelNum === void 0) {
  ...
  debug = (...values) => this.log(Level.DEBUG, values);
  info = (...values) => this.log(Level.INFO, values);
  notice = (...values) => this.log(Level.NOTICE, values);
  warn = (...values) => this.log(Level.WARN, values);
  error = (...values) => this.log(Level.ERROR, values);
  fatal = (...values) => this.log(Level.FATAL, values);
  log(level, values) {
