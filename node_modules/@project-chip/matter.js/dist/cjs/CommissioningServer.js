...
var import_AccessControlServer = require("./cluster/server/AccessControlServer.js");
...
var import_GeneralDiagnosticsServer = require("./cluster/server/GeneralDiagnosticsServer.js");
...
var import_LegacyInteractionServer = require("./device/LegacyInteractionServer.js");
...
class CommissioningServer extends import_MatterNode.MatterNode {
  ...
  constructor(options) {
    ...
    this.flowType = flowType ?? import_PairingCodeSchema.CommissioningFlowType.Standard;
    ...
    const basicInformationAttributes = {
      ...
      serialNumber: `matter.js-${import_Crypto.Crypto.get().getRandomData(4).toHex()}`,
    ...
    if (certificates == void 0) {
      ...
      const declaration = import_CertificationDeclarationManager.CertificationDeclarationManager.generate(vendorId, productId);
      certificates = {
    ...
    this.rootEndpoint.addClusterServer(
      (0, import_ClusterServer.ClusterServer)(
        ...
        (0, import_OperationalCredentialsServer.OperationalCredentialsClusterHandler)(certificates, this.productDescription)
      )
    );
    this.rootEndpoint.addClusterServer(
      (0, import_ClusterServer.ClusterServer)(
    ...
    this.rootEndpoint.addClusterServer((0, import_AccessControlServer.createDefaultAccessControlClusterServer)());
    this.rootEndpoint.addClusterServer(
      (0, import_ClusterServer.ClusterServer)(
    ...
    this.endpointStructure.change.on(() => {
      if (this.storage === void 0 || this.eventHandler === void 0) {
        throw new import_MatterError.InternalError("Endpoint structure reports change prior to server initialization");
      }
  ...
  endpointStructure;
  interactionServer;
  rootEndpoint = new import_Device.RootEndpoint();
  nextEndpointId;
  delayedAnnouncement;
  commandHandler = new import_NamedHandler.NamedHandler();
  /**
  ...
  async advertise(limitTo) {
    ...
    this.rootEndpoint.addClusterServer(await (0, import_GeneralDiagnosticsServer.createDefaultGeneralDiagnosticsClusterServer)(this.commandHandler));
    ...
    this.interactionServer = new import_LegacyInteractionServer.LegacyInteractionServer({
    ...
    const deviceInstance = await import_MatterDevice.MatterDevice.create(
    ...
    deviceInstance.addTransportInterface(
      await import_UdpInterface.UdpInterface.create(import_Network.Network.get(), "udp6", this.port, this.options.listeningAddressIpv6)
    );
    ...
    if (!this.ipv4Disabled) {
      deviceInstance.addTransportInterface(
        await import_UdpInterface.UdpInterface.create(import_Network.Network.get(), "udp4", this.port, this.options.listeningAddressIpv4)
      );
    }
    if (this.isCommissioned()) {
      const fabrics = deviceInstance.getFabrics();
      for (const fabric of fabrics) {
        if ((0, import_AttributeServer.genericFabricScopedAttributeGetterFromFabric)(
          fabric,
          import_AccessControlCluster.AccessControl.Cluster,
          "acl",
          void 0
        ) === void 0) {
          (0, import_AttributeServer.genericFabricScopedAttributeSetterForFabric)(fabric, import_AccessControlCluster.AccessControl.Cluster, "acl", [
            {
  ...
  initializeEndpointIdsFromStorage(endpoint, parentUniquePrefix = "") {
    ...
    for (let endpointIndex = 0; endpointIndex < endpoints.length; endpointIndex++) {
      ...
      if (endpoint2.number !== void 0 && endpoint2.number > this.nextEndpointId) {
        this.nextEndpointId = (0, import_EndpointNumber.EndpointNumber)(endpoint2.number + 1);
      }
  ...
  fillAndStoreEndpointIds(endpoint, parentUniquePrefix = "") {
    ...
    for (let endpointIndex = 0; endpointIndex < endpoints.length; endpointIndex++) {
      ...
      if (endpoint.number === void 0) {
        endpoint.number = (0, import_EndpointNumber.EndpointNumber)(this.nextEndpointId++);
  ...
  async setStorage(storage) {
    ...
    this.eventHandler = await import_EventHandler.EventHandler.create(this.storage.createContext("EventHandler"));
  }
  /**
  ...
  setPort(port) {
    ...
    if (this.deviceInstance !== void 0 || this.mdnsInstanceBroadcaster !== void 0) {
      throw new import_MatterError.ImplementationError("Port cannot be changed after device is initialized!");
    }
    this.port = port;
  }
  /**
...
class CommissioningServerClusterDatasource {
  ...
  #storage;
  #eventHandler;
  constructor(endpoint, cluster, storage, eventHandler) {
    ...
    this.#version = cluster.datasource?.version ?? import_Crypto.Crypto.getRandomUInt32();
