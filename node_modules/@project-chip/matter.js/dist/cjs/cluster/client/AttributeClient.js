...
var import_FabricIndex2 = require("../../model/standard/elements/FabricIndex.js");
...
class AttributeClient {
  ...
  async set(value, dataVersion) {
    if (!this.#isWritable) throw new import_Cluster.AttributeError(`Attribute ${this.name} is not writable`);
    value = this.schema.injectField(
      value,
      import_FabricIndex2.FabricIndex.id,
      import_FabricIndex.FabricIndex.OMIT_FABRIC,
    ...
    if (this.#isFabricScoped) {
      value = this.schema.removeField(
        value,
        import_FabricIndex2.FabricIndex.id,
        (existingFieldIndex) => existingFieldIndex === import_FabricIndex.FabricIndex.OMIT_FABRIC
      );
      try {
  ...
  async subscribe(minIntervalFloorSeconds, maxIntervalCeilingSeconds, knownDataVersion, isFabricFiltered = true) {
    if (!this.#updatedBySubscriptions) {
      throw new import_MatterError.ImplementationError(`Attribute ${this.name} is not updated by subscriptions.`);
    }
