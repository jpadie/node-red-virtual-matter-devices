...
function ClusterClient(clusterDef, endpointId, interactionClient, globalAttributeValues = {}) {
  function addAttributeToResult(attribute, attributeName) {
    ...
    result[`get${capitalizedAttributeName}Attribute`] = async (alwaysRequestFromRemote, isFabricFiltered = true) => {
      try {
        ...
        if (import_StatusCode.StatusResponseError.is(e, import_StatusCode.StatusCode.UnsupportedAttribute)) {
          return void 0;
        }
        throw e;
      }
  ...
  function addEventToResult(event, eventName) {
    ...
    result[`get${capitalizedEventName}Event`] = async (minimumEventNumber, isFabricFiltered) => {
      try {
        ...
        if (import_StatusCode.StatusResponseError.is(e, import_StatusCode.StatusCode.UnsupportedEvent)) {
          return void 0;
        }
        throw e;
      }
  ...
  const attributeToId = {};
  const allAttributeDefs = (0, import_Type.Merge)(
    attributeDef,
    (0, import_Cluster.GlobalAttributes)(features)
  );
  ...
  if (globalAttributeValues?.attributeList !== void 0) {
    for (const attributeId of globalAttributeValues.attributeList) {
      if (attributeToId[attributeId] === void 0) {
        ...
        addAttributeToResult(attribute, `unknownAttribute_${import_Diagnostic.Diagnostic.hex(attributeId)}`);
        logger.info(
          `Added unknown attribute ${import_Diagnostic.Diagnostic.hex(attributeId)} to cluster ${import_Diagnostic.Diagnostic.hex(clusterId)}`
        );
      }
    }
  }
  ...
  if (globalAttributeValues?.eventList !== void 0) {
    for (const eventId of globalAttributeValues.eventList) {
      if (eventToId[eventId] === void 0) {
        ...
        addEventToResult(event, `unknownEvent_${import_Diagnostic.Diagnostic.hex(eventId)}`);
        logger.info(`Added unknown event ${import_Diagnostic.Diagnostic.hex(eventId)} to cluster ${import_Diagnostic.Diagnostic.hex(clusterId)}.`);
      }
    }
  }
  const commandToId = {};
  ...
  if (globalAttributeValues?.acceptedCommandList !== void 0) {
    for (const requestId of globalAttributeValues.acceptedCommandList) {
      if (commandToId[requestId] === void 0) {
        logger.info(`Ignoring unknown command ${requestId} at cluster ${import_Diagnostic.Diagnostic.hex(clusterId)}`);
      }
    }
  }
  return result;
