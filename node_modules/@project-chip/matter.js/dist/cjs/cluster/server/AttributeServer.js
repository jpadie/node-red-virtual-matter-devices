...
var import_FabricIndex = require("../../model/standard/elements/FabricIndex.js");
...
const FabricIndexName = (0, import_String.camelize)(import_FabricIndex.FabricIndex.name);
...
class BaseAttributeServer {
  constructor(id, name, readAcl, writeAcl, schema, isWritable, isSubscribable, requiresTimedInteraction, initValue, defaultValue) {
    ...
    try {
      ...
      logger.warn(
        `Attribute value to initialize for ${name} has an invalid value ${import_Logger.Logger.toJSON(
          initValue
        )}. Restore to default ${import_Logger.Logger.toJSON(defaultValue)}`
      );
      if (defaultValue === void 0) {
        throw new import_MatterError.ImplementationError(`Attribute value to initialize for ${name} cannot be undefined.`);
      }
  ...
  get writeAcl() {
    return this.#writeAcl ?? import_Cluster.AccessLevel.Operate;
  }
  get readAcl() {
    return this.#readAcl ?? import_Cluster.AccessLevel.View;
  }
}
class FixedAttributeServer extends BaseAttributeServer {
  ...
  init(value) {
    if (value === void 0) {
      throw new import_MatterError.InternalError(`Cannot initialize fixed attribute "${this.name}" with undefined value.`);
    }
...
class AttributeServer extends FixedAttributeServer {
  ...
  init(value) {
    ...
    if (value === void 0) {
      throw new import_MatterError.InternalError(`Cannot initialize attribute "${this.name}" with undefined value.`);
    }
  ...
  processSet(value, session, message, delayChangeEvents = false) {
    ...
    if (delayChangeEvents) {
      ...
      logger.info(`Delay change for attribute "${this.name}" with value ${import_Logger.Logger.toJSON(value)}`);
    } else {
  ...
  triggerDelayedChangeEvents() {
    if (this.delayedChangeData !== void 0) {
      ...
      logger.info(`Trigger delayed change for attribute "${this.name}" with value ${import_Logger.Logger.toJSON(newValue)}`);
...
function genericFabricScopedAttributeGetter(session, isFabricFiltered, cluster, attributeName, defaultValue) {
  if (session === void 0) {
...
function genericFabricScopedAttributeSetterForFabric(fabric, cluster, attributeName, value, defaultValue) {
  ...
  if (!(0, import_DeepEqual.isDeepEqual)(value, oldValue)) {
    ...
    if (import_Promises.MaybePromise.is(setResult)) {
      throw new import_MatterError.ImplementationError(
...
class FabricScopedAttributeServer extends AttributeServer {
  ...
  init(value) {
    if (value !== void 0) {
      throw new import_MatterError.InternalError(`Cannot initialize fabric scoped attribute "${this.name}" with a value.`);
    }
  }
  /**
  ...
  setRemote(value, session, message, delayChangeEvents = false, preserveFabricIndex = false) {
    value = this.schema.injectField(
      value,
      import_FabricIndex.FabricIndex.id,
    ...
    logger.info(`Set remote value for fabric scoped attribute "${this.name}" to ${import_Logger.Logger.toJSON(value)}`);
