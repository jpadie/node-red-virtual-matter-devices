...
var import_NetworkCommissioningCluster = require("../definitions/NetworkCommissioningCluster.js");
 ...
 */
class CommissioningServerFailsafeContext extends import_FailsafeContext.FailsafeContext {
  #rootEndpoint;
  ...
  static async create(rootEndpoint, options) {
    return (0, import_Construction.asyncNew)(this, rootEndpoint, options);
  }
  ...
  async restoreFabric(fabric) {
    ...
    const operationalCredentialsCluster = this.#rootEndpoint.getClusterServer(import_OperationalCredentialsCluster.OperationalCredentials.Cluster);
  ...
  async revokeFabric(fabric) {
    const basicInformationCluster = this.#rootEndpoint.getClusterServer(import_BasicInformationCluster.BasicInformation.Cluster);
  ...
  async restoreBreadcrumb() {
    const generalCommissioningCluster = this.#rootEndpoint.getClusterServer(import_GeneralCommissioningCluster.GeneralCommissioning.Cluster);
    generalCommissioningCluster?.setBreadcrumbAttribute(0);
  }
  async #storeEndpointState(endpoint) {
    const networkCluster = endpoint.getClusterServer(import_NetworkCommissioningCluster.NetworkCommissioning.Complete);
