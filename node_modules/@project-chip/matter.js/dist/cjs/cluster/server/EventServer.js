...
class EventServer {
  ...
  bindToEventHandler(eventHandler) {
    ...
    for (const event of this.eventList) {
      ...
      if (finalEvent !== void 0 && import_Promises.MaybePromise.is(finalEvent)) {
  ...
  triggerEvent(data) {
    ...
    if (this.eventHandler === void 0) {
      ...
      const finalEvent = this.eventHandler.pushEvent(event);
      return import_Promises.MaybePromise.then(finalEvent, (e) => {
  ...
  get(session, isFabricFiltered, _message, filters) {
    if (this.eventHandler === void 0) {
      throw new import_MatterError.InternalError("EventServer not bound to EventHandler");
    }
    if (this.endpoint === void 0) {
      throw new import_MatterError.InternalError("EventServer not bound to Endpoint");
    }
    return this.eventHandler.getEvents(
      ...
      isFabricFiltered ? session.fabric?.fabricIndex ?? import_FabricIndex.FabricIndex.NO_FABRIC : void 0
    );
  }
}
class FabricSensitiveEventServer extends EventServer {
  ...
  triggerEvent(data) {
    if (!(0, import_Type.isObject)(data) || data === null) {
      throw new import_MatterError.ImplementationError("FabricSensitive events need to have an object as data.");
    }
    if (!("fabricIndex" in data)) {
      throw new import_MatterError.InternalError("FabricSensitive events requires fabricIndex in data.");
    }
