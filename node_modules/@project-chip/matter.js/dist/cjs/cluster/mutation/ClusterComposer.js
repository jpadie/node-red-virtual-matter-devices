 ...
 */
class IllegalClusterError extends import_MatterError.MatterError {
}
class ClusterComposer {
  ...
  compose(selection) {
    ...
    if (extensions) {
      ...
      if ((0, import_DeepEqual.isDeepEqual)(supportedFeatures, this.cluster.supportedFeatures)) {
  ...
  validateFeatureSelection(features) {
    for (const f of features) {
      if (!this.cluster.features[(0, import_String.camelize)(f)]) {
  ...
  reject(definition, flags) {
    ...
    throw new IllegalClusterError(
      `Feature combination ${(0, import_String.serialize)(flags)} is disallowed by the Matter specification`
    );
  }
}
