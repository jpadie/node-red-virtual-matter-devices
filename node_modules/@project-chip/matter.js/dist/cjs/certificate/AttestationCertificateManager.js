...
class AttestationCertificateManager {
  ...
  paaKeyPair = (0, import_Key.PrivateKey)(import_ChipPAAuthorities.TestCert_PAA_NoVID_PrivateKey, {
  ...
  paiCertId = BigInt(1);
  paiKeyPair = import_Crypto.Crypto.createKeyPair();
  paiKeyIdentifier = import_Crypto.Crypto.hash(this.paiKeyPair.publicKey).slice(0, 20);
  paiCertBytes;
  nextCertificateId = 2;
  getPAICert() {
  ...
  generatePAACert(vendorId) {
    ...
    const unsignedCertificate = {
      serialNumber: import_ByteArray.ByteArray.fromHex((0, import_Number.toHex)(this.paaCertId)),
  ...
  generatePAICert(vendorId, productId) {
    const now = import_Time.Time.get().now();
    const unsignedCertificate = {
      serialNumber: import_ByteArray.ByteArray.fromHex((0, import_Number.toHex)(this.paiCertId)),
      signatureAlgorithm: 1,
      publicKeyAlgorithm: 1,
      ellipticCurveIdentifier: 1,
      issuer: {
