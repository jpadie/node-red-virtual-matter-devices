...
((Format2) => {
  ...
  Format2.ANSI = "ansi";
  Format2.HTML = "html";
  Format2.plain = formatPlain;
  Format2.ansi = formatAnsi;
  Format2.html = formatHtml;
})(Format || (Format = {}));
...
function formatPlain(diagnostic, indents = 0) {
  ...
  const formatter = {
    ...
    message: (message) => {
      ...
      return `${formatTime(message.now)} ${import_Level.Level[message.level]} ${message.facility} ${message.prefix}${formattedValues}`;
    },
...
function formatAnsi(diagnostic, indents = 0) {
  ...
  const formatter = {
    message: ({ now, level, facility, prefix: nestPrefix, values }) => {
      ...
      styles[0] = (import_Level.Level[level] ?? "default").toLowerCase();
      const prefix = style("prefix", `${formatTime(now)} ${import_Level.Level[level].padEnd(6)}`);
      facility = style(
        "facility",
...
function formatHtml(diagnostic) {
  ...
  const formatter = {
    message: ({ now, level, facility, prefix, values }) => {
      ...
      return htmlSpan(
        `line ${import_Level.Level[level].toLowerCase()}`,
        `${htmlSpan("time", formatTime(now))} ${htmlSpan("level", import_Level.Level[level])} ${htmlSpan(
          "facility",
          facility
...
if (import_MatterError.MatterError.formatterFor === import_MatterError.MatterError.defaultFormatterFactory) {
  import_MatterError.MatterError.formatterFor = Format;
}
