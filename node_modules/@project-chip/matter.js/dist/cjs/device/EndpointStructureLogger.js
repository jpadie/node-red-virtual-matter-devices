...
function logClusterServer(endpoint, clusterServer, options = {}) {
  ...
  logger.info(
    `Cluster-Server "${clusterServer.name}" (${import_Diagnostic.Diagnostic.hex(clusterServer.id)}) ${supportedFeatures.length ? `(Features: ${supportedFeatures.join(", ")})` : ""}`
  );
    ...
    import_Logger.Logger.nest(() => {
      ...
      import_Logger.Logger.nest(() => {
        for (const attributeName in globalAttributes) {
          ...
          logger.info(
            `"${attribute.name}" (${import_Diagnostic.Diagnostic.hex(attribute.id)})${value !== "" ? `: value = ${value}` : ""}`
          );
        }
      });
    });
    ...
    import_Logger.Logger.nest(() => {
      ...
      import_Logger.Logger.nest(() => {
        ...
        for (const commandName in commands) {
          ...
          logger.info(
            `"${command.name}" (${import_Diagnostic.Diagnostic.hex(command.invokeId)}/${import_Diagnostic.Diagnostic.hex(command.responseId)})`
          );
        }
      });
    });
    ...
    import_Logger.Logger.nest(() => {
      ...
      import_Logger.Logger.nest(() => {
        ...
        for (const eventName in events) {
          ...
          logger.info(`"${event.name}" (${import_Diagnostic.Diagnostic.hex(event.id)})`);
        }
      });
    });
  }
...
function logClusterClient(endpoint, clusterClient, options = {}) {
  ...
  logger.info(
    `Cluster-Client "${clusterClient.name}" (${import_Diagnostic.Diagnostic.hex(clusterClient.id)}) ${supportedFeatures.length ? `(Features: ${supportedFeatures.join(", ")})` : ""}`
  );
  if (options.logClusterGlobalAttributes !== false) {
    import_Logger.Logger.nest(() => {
      logger.info("Global-Attributes:");
      import_Logger.Logger.nest(() => {
        for (const attributeName in globalAttributes) {
          ...
          if (attribute === void 0) continue;
          logger.info(`"${attribute.name}" (${import_Diagnostic.Diagnostic.hex(attribute.id)})`);
        }
      });
    });
  }
    ...
    import_Logger.Logger.nest(() => {
      ...
      import_Logger.Logger.nest(() => {
        for (const attributeName in clusterClient.attributes) {
          ...
          logger.info(`"${attribute.name}" (${import_Diagnostic.Diagnostic.hex(attribute.id)})${info}`);
        }
      });
    });
  }
    ...
    import_Logger.Logger.nest(() => {
      ...
      import_Logger.Logger.nest(() => {
        for (const eventName in clusterClient.events) {
          ...
          if (!supported) info += " (Not Supported)";
          if (unknown) info += " (Unknown)";
          logger.info(`"${event.name}" (${import_Diagnostic.Diagnostic.hex(event.id)})${info}`);
        }
      });
    });
  }
