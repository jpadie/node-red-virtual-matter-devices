...
class MdnsScanner {
  ...
  async #sendQueries() {
    ...
    this.#queryTimer = import_Time.Time.getTimer(
      "MDNS discovery",
      ...
      if (answersToSend.length > 0) {
        ...
        if (dnsMessageSize > import_DnsCodec.MAX_MDNS_MESSAGE_SIZE) {
          if (dnsMessageDataToSend.answers.length === 0) {
            logger.warn(
              `MDNS Query with ${import_Logger.Logger.toJSON(
                queries
  ...
  #setQueryRecords(queryId, queries, answers = []) {
    ...
    logger.debug(`Set ${queries.length} query records for query ${queryId}: ${import_Logger.Logger.toJSON(queries)}`);
    this.#queryTimer?.stop();
    this.#nextAnnounceIntervalSeconds = START_ANNOUNCE_INTERVAL_SECONDS;
    this.#queryTimer = import_Time.Time.getTimer("MDNS discovery", 0, () => this.#sendQueries()).start();
  }
  ...
  async #registerWaiterPromise(queryId, timeoutSeconds, resolveOnUpdatedRecords = true) {
    ...
    const timer = timeoutSeconds !== void 0 ? import_Time.Time.getTimer("MDNS timeout", timeoutSeconds * 1e3, () => this.#finishWaiter(queryId, true)).start() : void 0;
  ...
  #buildCommissionableQueryIdentifier(identifier) {
    ...
    throw new import_MatterError.ImplementationError(`Invalid commissionable device identifier : ${import_Logger.Logger.toJSON(identifier)}`);
  }
  ...
  #handleCommissionableRecords(answers, formerAnswers, netInterface) {
    ...
    for (const record of txtRecords) {
      ...
      if (parsedRecord === void 0) continue;
  ...
  #parseCommissionableTxtRecord(record) {
    ...
    if (result.D === void 0 || result.CM === void 0) return void 0;
    return result;
  }
  #expire() {
  ...
  static deviceAddressDiagnostics(addresses) {
    return Array.from(addresses.values()).map(
      (address) => import_Diagnostic.Diagnostic.dict({
        ip: address.ip,
        port: address.port,
        type: address.type
      })
