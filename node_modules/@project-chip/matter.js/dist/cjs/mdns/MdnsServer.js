...
class MdnsServer {
  ...
  #records = new import_Cache.AsyncCache(
    "MDNS discovery",
  ...
  async #handleDnsMessage(messageBytes, remoteIp, netInterface) {
    ...
    if (messageType !== import_DnsCodec.DnsMessageType.Query && messageType !== import_DnsCodec.DnsMessageType.TruncatedQuery) return;
    ...
    for (const portRecords of records.values()) {
      ...
      await import_Time.Time.sleep("MDNS delay", 20 + Math.floor(Math.random() * 100));
    }
  }
      ...
      if (answersToSend.length > 0) {
        ...
        if (dnsMessageSize > import_DnsCodec.MAX_MDNS_MESSAGE_SIZE) {
          await this.#multicastServer.send(
            import_DnsCodec.DnsCodec.encode(dnsMessageDataToSend),
            netInterface,
            unicastTarget
          );
