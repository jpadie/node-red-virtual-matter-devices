...
class MessageCodec {
  ...
  static decodePacketHeader(reader) {
    ...
    if (version !== HEADER_VERSION) throw new import_MatterError.NotImplementedError(`Unsupported header version ${version}.`);
    ...
    if (hasPrivacyEnhancements) throw new import_MatterError.NotImplementedError(`Privacy enhancements not supported`);
    const isControlMessage = (securityFlags & 64 /* IsControlMessage */) !== 0;
    if (isControlMessage) throw new import_MatterError.NotImplementedError(`Control Messages not supported`);
