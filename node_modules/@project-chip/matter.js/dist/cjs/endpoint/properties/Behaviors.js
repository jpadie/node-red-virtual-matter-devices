...
var import_DescriptorServer = require("../../behavior/definitions/descriptor/DescriptorServer.js");
 ...
 */
const logger = import_Logger.Logger.get("Behaviors");
class Behaviors {
  #endpoint;
  #supported;
  #backings = {};
  ...
  get status() {
    ...
    for (const key in this.#supported) {
      status[key] = this.#backings[key]?.status ?? import_Lifecycle.Lifecycle.Status.Inactive;
    }
    return status;
  }
  get [import_Diagnostic.Diagnostic.value]() {
    return import_Diagnostic.Diagnostic.lifecycleList(this.status);
  }
  get detailedDiagnostic() {
    return Object.entries(this.#supported).map(([name, type]) => {
      const backing = this.#backings[name];
      const result = [(0, import_Diagnostic.Diagnostic)(backing?.status ?? import_Lifecycle.Lifecycle.Status.Inactive, name)];
      ...
      if (features.size) {
        elementDiagnostic.push([import_Diagnostic.Diagnostic.strong("features"), features]);
      }
      if (elements.attributes.size) {
        elementDiagnostic.push([import_Diagnostic.Diagnostic.strong("attributes"), elements.attributes]);
      }
      if (elements.commands.size) {
        elementDiagnostic.push([import_Diagnostic.Diagnostic.strong("commands"), elements.commands]);
      }
      if (elements.events.size) {
        elementDiagnostic.push([import_Diagnostic.Diagnostic.strong("events"), elements.events]);
      }
      if (elementDiagnostic.length) {
        result.push(import_Diagnostic.Diagnostic.list(elementDiagnostic));
      }
      return result;
    });
  }
  constructor(endpoint, options) {
    ...
    if (typeof type?.behaviors !== "object") {
      throw new import_MatterError.ImplementationError('EndpointType "behaviors" must be an array of Behavior.Type instances');
    }
    ...
    for (const id in this.#supported) {
      ...
      if (!(type2.prototype instanceof import_Behavior.Behavior)) {
        throw new import_MatterError.ImplementationError(`${endpoint}.${id}" is not a Behavior.Type`);
      }
      if (typeof type2.id !== "string") {
        throw new import_MatterError.ImplementationError(`${endpoint}.${id} has no ID`);
      }
  ...
  initialize() {
    if (!this.#endpoint.lifecycle.isInstalled) {
      throw new import_MatterError.ImplementationError(`Cannot initialize behaviors because endpoint is not installed`);
    }
    const initializeBehaviors = (context) => {
      ...
      return import_Construction.Construction.all(
        {
          [Symbol.iterator]: () => {
  ...
  createSync(type, agent) {
    ...
    if (import_Promises.MaybePromise.is(behavior)) {
      throw new import_MatterError.ImplementationError(
  ...
  createMaybeAsync(type, agent) {
    ...
    if (backing.construction.status === import_Lifecycle.Lifecycle.Status.Initializing) {
    ...
    const dispose = async (context) => {
      ...
      while (destroyNow.size) {
        for (const key in this.#backings) {
          ...
          if (!destroyNow.size) {
            throw new import_MatterError.ImplementationError("Cannot destroy behaviors due to circular dependency");
          }
        }
  ...
  validateRequirements(requirements) {
    ...
    for (const requirement of Object.values(requirements)) {
      let name = (0, import_String.camelize)(requirement.name, true);
    ...
    if (missing.length) {
      throw new import_MatterError.ImplementationError(
        `${this.#endpoint} is missing required behaviors: ${(0, import_String.describeList)("and", ...missing)}`
      );
    }
  }
  /**
  ...
  internalsOf(type) {
    ...
    if (!backing) {
      ...
      if (backing === void 0) {
        throw new import_MatterError.InternalError(`Behavior ${this.#endpoint}.${type.id} late activation did not create backing`);
      }
    }
  ...
  #backingFor(container, type) {
    if (this.#endpoint.construction.status !== import_Lifecycle.Lifecycle.Status.Initializing) {
  ...
  #getBehaviorType(type) {
    ...
    if (typeof myType !== "function" || !(myType.prototype instanceof import_Behavior.Behavior)) {
      throw new import_MatterError.ImplementationError(`Endpoint behavior "${type.id}" implementation is not a Behavior`);
    }
    return myType;
  }
  /**
  ...
  #augmentEndpoint(type) {
    Object.defineProperty(this.#endpoint.state, type.id, {
      ...
      set() {
        throw new import_MatterError.ReadOnlyError('The "state" property is read-only; you must use set() to modify state');
      },
      enumerable: true
    });
    let events;
