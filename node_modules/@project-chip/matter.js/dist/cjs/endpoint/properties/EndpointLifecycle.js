 ...
 */
const logger = import_Logger.Logger.get("PartLifecycle");
class EndpointLifecycle {
  ...
  #hasId = false;
  #hasNumber = false;
  #installed = (0, import_Observable.Observable)((error) => this.emitError("installed", error));
  #ready = (0, import_Observable.Observable)((error) => this.emitError("ready", error));
  #partsReady = (0, import_Observable.Observable)((error) => this.emitError("partsReady", error));
  #destroyed = (0, import_Observable.Observable)((error) => this.emitError("destroyed", error));
  #changed = (0, import_Observable.Observable)(
    (error) => this.emitError("changed", error)
  );
  #reset = (0, import_Observable.AsyncObservable)();
  #queuedUpdates;
  /**
  ...
  constructor(endpoint, isEssential) {
    ...
    endpoint.construction.change.on((status) => {
      if (status === import_Lifecycle.Lifecycle.Status.Crashed) {
  ...
  change(type) {
    switch (type) {
      ...
      case EndpointLifecycle.Change.Ready:
        if (!this.#hasId) {
          throw new import_MatterError.ImplementationError("Endpoint reports as ready but has no ID assigned");
        }
        if (!this.#hasNumber) {
          throw new import_MatterError.ImplementationError("Endpoint reports as ready but has no number assigned");
        }
        this.#isReady = true;
        break;
      case EndpointLifecycle.Change.PartsReady:
        if (!this.#isReady) {
          throw new import_MatterError.ImplementationError("Endpoint reports as parts-ready but is not itself ready");
        }
  ...
  emitError(name, error) {
    logger.error("Unhandled error in", import_Diagnostic.Diagnostic.strong(`${this.#endpoint}.lifecycle.${name}`), "handler:", error);
  }
  /**
