...
var import_ServerBacking = require("../behavior/internal/ServerBacking.js");
...
class EndpointServer {
  ...
  createBacking(type) {
    ...
    if (type.prototype instanceof import_ClusterBehavior.ClusterBehavior) {
      ...
      if (this.#clusterServers.has(cluster.id)) {
        throw new import_MatterError.InternalError(`${this.#endpoint}.${cluster.id} initialized multiple times`);
      }
      backing = new import_ClusterServerBacking.ClusterServerBacking(this, type);
    } else {
      backing = new import_ServerBacking.ServerBehaviorBacking(this.#endpoint, type);
    }
    return backing;
  }
  get number() {
  ...
  getNumber() {
    if (this.number === void 0) {
      throw new import_MatterError.InternalError("Endpoint ID has not been assigned yet");
    }
    return this.number;
  }
  removeFromStructure() {
  ...
  addClusterClient() {
    throw new import_MatterError.NotImplementedError("Cluster clients unavailable on EndpointServer");
  }
  getClusterClient() {
    throw new import_MatterError.NotImplementedError("Cluster clients unavailable on EndpointServer");
  }
  addChildEndpoint(endpoint) {
    if (endpoint instanceof EndpointServer) {
      ...
      throw new import_MatterError.ImplementationError("Attempt to add unmanaged endpoint as child of Endpoint");
    }
  }
  getChildEndpoint(id) {
    if (!this.#endpoint.hasParts) {
