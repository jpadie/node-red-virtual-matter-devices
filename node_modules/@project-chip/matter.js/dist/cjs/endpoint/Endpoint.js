...
var import_RootEndpoint = require("./definitions/system/RootEndpoint.js");
 ...
 */
const logger = import_Logger.Logger.get("Endpoint");
class Endpoint {
  #type;
  #id;
  #number;
  ...
  get id() {
    if (this.#id === void 0) {
      throw new import_Lifecycle.UninitializedDependencyError(
        this.toString(),
  ...
  get env() {
    ...
    return import_Environment.Environment.default;
  }
  /**
  ...
  stateOf(type) {
    if (!this.behaviors.has(type)) {
      throw new import_MatterError.ImplementationError(`Behavior ${type.id} is not supported by ${this}`);
    }
    ...
    await this.act(`set<${this}>`, async (agent) => {
      ...
      for (const behaviorId in values) {
        ...
        if (!(behavior instanceof import_Behavior.Behavior)) {
          throw new import_MatterError.ImplementationError(`Behavior ID ${behaviorId} does not exist`);
        }
        ...
        if (Array.isArray(vals)) {
          throw new import_MatterError.ImplementationError(`StateValue for ${behaviorId} must be an object, not an array`);
        }
  ...
  async setStateOf(type, values) {
    await this.act(`setStateOf<${this}>`, async (agent) => {
      ...
      if (typeof values !== "object") {
        throw new import_MatterError.ImplementationError(`State values for ${type.id} must be an object, not ${typeof values}`);
      }
      if (Array.isArray(values)) {
        throw new import_MatterError.ImplementationError(`State values for ${type.id} must be an object, not an array`);
      }
  ...
  eventsOf(type) {
    if (!this.behaviors.has(type)) {
      throw new import_MatterError.ImplementationError(`Behavior ${type.id} is not supported by this endpoint`);
    }
  ...
  set id(id) {
    ...
    if (this.#id !== void 0) {
      throw new import_MatterError.ImplementationError(`${this} ID is already assigned, cannot reassign`);
    }
    if (typeof id !== "string") {
      throw new import_MatterError.ImplementationError(`Illegal endpoint ID type "${typeof id}"`);
    }
    if (id === "") {
      throw new import_MatterError.ImplementationError("Endpoint ID may not be empty");
    }
    if (id.includes(".")) {
      throw new import_MatterError.ImplementationError('Endpoint ID may not include "."');
    }
  ...
  set number(number) {
    ...
    if (typeof number !== "number") {
      throw new import_MatterError.ImplementationError(`Illegal endpoint number type "${typeof number}"`);
    }
    if (!Number.isInteger(number)) {
      throw new import_MatterError.ImplementationError(`Endpoint number ${number} is not an integer`);
    }
    if (number < 0) {
      throw new import_MatterError.ImplementationError(`Endpoint number ${number} is negative`);
    }
    if (number > 65535) {
      throw new import_MatterError.ImplementationError(`Endpoint number ${number} is greater than the maximum of 65535`);
    }
    if (this.type.deviceClass === import_RootEndpoint.RootEndpoint.deviceClass) {
      if (number !== 0) {
        throw new import_MatterError.ImplementationError("The root endpoint must have ID 0");
      }
    } else {
      if (number === 0) {
        throw new import_MatterError.ImplementationError("Only root endpoint may have ID 0");
      }
      if (this.lifecycle.isInstalled) {
        this.env.get(import_IdentityService.IdentityService).assertNumberAvailable(number, this);
      }
    }
    this.#number = (0, import_EndpointNumber.EndpointNumber)(number);
  ...
  set owner(owner) {
    ...
    if (this.#owner) {
      throw new import_MatterError.ImplementationError("Endpoint owner cannot be reassigned");
    }
    if (owner === void 0) {
      throw new import_MatterError.ImplementationError("Endpoint owner must be defined");
    }
    this.#owner = owner;
    try {
  ...
  async add(definition, options) {
    if (!this.lifecycle.isInstalled) {
      throw new import_MatterError.ImplementationError(`You may not use add() here because ${this} is not installed in a Node`);
    }
  ...
  act(actorOrPurpose, actor) {
    ...
    if (typeof actor !== "function") {
      throw new import_MatterError.ImplementationError("Actor must be a function");
    }
  ...
  async reset() {
    try {
      ...
      this.construction.setStatus(import_Lifecycle.Lifecycle.Status.Inactive);
    } catch (e) {
  ...
  visit(visitor) {
    ...
    const visitChildren = () => {
      for (let next = childIterator.next(); !next.done; next = childIterator.next()) {
        ...
        if (import_Promises.MaybePromise.is(promise2)) {
  ...
  get path() {
    ...
    return (0, import_DataModelPath.DataModelPath)(ident, this.type?.name);
  }
  /**
   ...
   */
  initialize() {
    this.env.get(import_EndpointInitializer.EndpointInitializer).initializeDescendent(this);
  ...
  assertConstructable() {
    if (this.#owner === void 0) {
      throw new import_MatterError.ImplementationError(`Endpoint construction initiated without owner`);
    }
    if (!this.#owner.#lifecycle.isInstalled) {
      throw new import_MatterError.ImplementationError(`Endpoint construction initiated with uninstalled owner`);
    }
  }
  /**
   * Complete initialization.  Invoked via {@link Construction#start} by the owner.
   */
  [import_Construction.Construction.construct]() {
    this.assertConstructable();
    this.lifecycle.change(import_EndpointLifecycle.EndpointLifecycle.Change.Installed);
    return this.initialize();
  }
  #logReady() {
    logger.info(import_Diagnostic.Diagnostic.strong(this.toString()), "ready", this.diagnosticDict);
  }
  /**
  ...
  get [import_Diagnostic.Diagnostic.value]() {
    return [
      import_Diagnostic.Diagnostic.strong(this.id),
      import_Diagnostic.Diagnostic.dict({
        ...this.#diagnosticProps,
        class: this.constructor.name
      }),
      import_Diagnostic.Diagnostic.list([...this.behaviors.detailedDiagnostic, ...this.parts])
    ];
  }
  /**
   ...
   */
  get diagnosticDict() {
    return import_Diagnostic.Diagnostic.dict({
      ...this.#diagnosticProps,
