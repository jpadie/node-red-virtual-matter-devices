...
var import_SwitchCluster = require("../../../cluster/definitions/SwitchCluster.js");
 ...
 */
const SwitchServerBase = import_SwitchBehavior.SwitchBehavior.for(import_SwitchCluster.Switch.Complete).with(
  import_SwitchCluster.Switch.Feature.LatchingSwitch,
  import_SwitchCluster.Switch.Feature.MomentarySwitch,
  import_SwitchCluster.Switch.Feature.MomentarySwitchRelease,
  import_SwitchCluster.Switch.Feature.MomentarySwitchLongPress,
  import_SwitchCluster.Switch.Feature.MomentarySwitchMultiPress
);
const schema = SwitchServerBase.schema?.clone();
schema.add(
  (0, import_FieldElement.FieldElement)({ name: "longPressDelay", type: "uint32", quality: "M", conformance: "MSL" }),
  (0, import_FieldElement.FieldElement)({ name: "multiPressDelay", type: "uint32", quality: "M", conformance: "MSM" }),
  (0, import_FieldElement.FieldElement)({ name: "momentaryNeutralPosition", type: "uint8", quality: "O", conformance: "MS" })
);
class SwitchServerLogic extends SwitchServerBase {
  ...
  #assertPositionInRange(position) {
    if (position < 0 || position >= this.state.numberOfPositions) {
      throw new import_StatusCode.StatusResponseError(`Position ${position} invalid`, import_StatusCode.StatusCode.ConstraintError);
    }
  }
  #debounceRawPosition(newPosition) {
    if (this.state.debounceDelay !== 0) {
      ...
      this.internal.debounceTimer = import_Time.Time.getTimer(
        "debounce",
  ...
  #handleSwitchPositionChange(newPosition) {
    ...
    if (this.features.momentarySwitchLongPress) {
      ...
      if (newPosition !== this.state.momentaryNeutralPosition) {
        ...
        this.internal.longPressTimer = import_Time.Time.getTimer(
          "longPress",
    ...
    if (this.features.momentarySwitchMultiPress) {
      ...
      this.internal.multiPressTimer = import_Time.Time.getTimer(
        "multiPress",
...
((SwitchServerLogic2) => {
  ...
  class Events extends SwitchServerBase.Events {
    rawPosition$Changed = (0, import_Observable.Observable)();
    rawPosition$Changing = (0, import_Observable.Observable)();
  }
  SwitchServerLogic2.Events = Events;
})(SwitchServerLogic || (SwitchServerLogic = {}));
class SwitchServer extends SwitchServerLogic.for((0, import_ClusterType.ClusterType)(import_SwitchCluster.Switch.Base)) {
}
