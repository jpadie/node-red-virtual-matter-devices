 ...
 */
const logger = import_Logger.Logger.get("BasicInformationServer");
...
class BasicInformationServer extends Base {
  initialize() {
    ...
    setDefault("vendorId", (0, import_VendorId.VendorId)(65521));
    ...
    if (Object.keys(defaultsSet).length) {
      logger.warn("Using development values for some BasicInformation attributes:", import_Diagnostic.Diagnostic.dict(defaultsSet));
    }
    setDefault("productLabel", state.productName);
    setDefault("nodeLabel", state.productName);
    setDefault("dataModelRevision", import_Specification.Specification.DATA_MODEL_REVISION);
    ...
    setDefault("specificationVersion", import_Specification.Specification.SPECIFICATION_VERSION);
    setDefault("maxPathsPerInvoke", import_InteractionServer.DEFAULT_MAX_PATHS_PER_INVOKE);
    ...
    if (this.state.reachable !== void 0 && this.events.reachable$Changed !== void 0) {
      if (this.events.reachableChanged === void 0) {
        this.events.reachableChanged = (0, import_Observable.Observable)();
      }
