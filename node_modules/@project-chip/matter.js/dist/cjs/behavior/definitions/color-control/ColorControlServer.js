...
var import_ColorControlCluster = require("../../../cluster/definitions/ColorControlCluster.js");
...
var import_GeneralDiagnosticsBehavior = require("../general-diagnostics/GeneralDiagnosticsBehavior.js");
var import_OnOffServer = require("../on-off/OnOffServer.js");
 ...
 */
const logger = import_Logger.Logger.get("ColorControlServer");
const ColorControlServerBase = import_ColorControlBehavior.ColorControlBehavior.with(
  import_ColorControlCluster.ColorControl.Feature.HueSaturation,
  import_ColorControlCluster.ColorControl.Feature.EnhancedHue,
  import_ColorControlCluster.ColorControl.Feature.ColorLoop,
  import_ColorControlCluster.ColorControl.Feature.Xy,
  import_ColorControlCluster.ColorControl.Feature.ColorTemperature
);
...
class ColorControlServerLogic extends ColorControlServerBase {
   ...
   */
  set hue(value) {
    this.state.currentHue = (0, import_Number.cropValueRange)(Math.round(value * 254 / 360), MIN_HUE_VALUE, MAX_HUE_VALUE);
  }
  ...
  set enhancedHue(value) {
    this.state.enhancedCurrentHue = (0, import_Number.cropValueRange)(value, MIN_HUE_VALUE, MAX_ENHANCED_HUE_VALUE);
  }
  ...
  set saturation(value) {
    this.state.currentSaturation = (0, import_Number.cropValueRange)(
   ...
   */
  set mireds(value) {
    this.state.colorTemperatureMireds = (0, import_Number.cropValueRange)(
      value,
  ...
  initialize() {
    ...
    if (this.features.colorTemperature && this.#getBootReason() !== import_definitions.GeneralDiagnostics.BootReason.SoftwareUpdateCompleted) {
      ...
      if (targetMiredsValue !== currentMiredsValue) {
        this.state.colorMode = import_ColorControlCluster.ColorControl.ColorMode.ColorTemperatureMireds;
        this.state.enhancedColorMode = import_ColorControlCluster.ColorControl.EnhancedColorMode.ColorTemperatureMireds;
  ...
  moveToHue({
    ...
    return import_Promises.MaybePromise.then(
      this.setColorMode(import_ColorControlCluster.ColorControl.ColorMode.CurrentHueAndCurrentSaturation),
  ...
  moveHue({ optionsMask, optionsOverride, moveMode, rate }) {
    ...
    return import_Promises.MaybePromise.then(this.setColorMode(import_ColorControlCluster.ColorControl.ColorMode.CurrentHueAndCurrentSaturation), () => {
      if (moveMode === import_ColorControlCluster.ColorControl.MoveMode.Stop) {
  ...
  moveHueLogic(moveMode, rate, isEnhancedHue = false) {
    ...
    if (!this.state.managedTransitionTimeHandling) {
      this.internal.stopHueTransition();
      if (isEnhancedHue) {
        this.state.enhancedCurrentHue = (0, import_Number.addValueWithOverflow)(
        ...
        this.state.currentHue = (0, import_Number.addValueWithOverflow)(this.state.currentHue, rate, MIN_HUE_VALUE, maxHue);
      }
      this.state.remainingTime = 0;
      return;
    }
    this.#initiateTransition(
      isEnhancedHue ? "enhancedHue" : "hue",
      rate * (moveMode === import_ColorControlCluster.ColorControl.MoveMode.Up ? 1 : -1),
      MIN_HUE_VALUE,
      maxHue
    );
  }
  ...
  stepHueLogic(stepMode, stepSize, transitionTime, isEnhancedHue = false) {
    const currentHue = isEnhancedHue ? this.state.enhancedCurrentHue : this.state.currentHue;
    const maxHue = isEnhancedHue ? MAX_ENHANCED_HUE_VALUE : MAX_HUE_VALUE;
    const targetValue = (0, import_Number.addValueWithOverflow)(
      currentHue,
      stepMode === import_ColorControlCluster.ColorControl.StepMode.Up ? stepSize : -stepSize,
      MIN_HUE_VALUE,
      maxHue
    );
    if (!this.state.managedTransitionTimeHandling || transitionTime === 0) {
    ...
    const effectiveRate = Math.ceil(stepSize / transitionTime * 10) * (stepMode === import_ColorControlCluster.ColorControl.StepMode.Up ? 1 : -1);
    this.#initiateTransition(
      isEnhancedHue ? "enhancedHue" : "hue",
      effectiveRate,
      MIN_HUE_VALUE,
  ...
  moveSaturationLogic(moveMode, rate) {
    if (!this.state.managedTransitionTimeHandling) {
      this.internal.stopSaturationTransition();
      this.state.currentSaturation = moveMode === import_ColorControlCluster.ColorControl.MoveMode.Up ? MAX_SATURATION_VALUE : MIN_SATURATION_VALUE;
      this.state.remainingTime = 0;
      return;
    }
    this.#initiateTransition(
  ...
  stepSaturationLogic(stepMode, stepSize, transitionTime) {
    const effectiveRate = Math.ceil(stepSize / transitionTime * 10) * (stepMode === import_ColorControlCluster.ColorControl.StepMode.Up ? 1 : -1);
    const targetValue = (0, import_Number.cropValueRange)(
      stepMode === import_ColorControlCluster.ColorControl.StepMode.Up ? this.state.currentSaturation + stepSize : this.state.currentSaturation - stepSize,
      MIN_SATURATION_VALUE,
      MAX_SATURATION_VALUE
    );
  ...
  moveToHueAndSaturationLogic(targetHue, targetSaturation, transitionTime) {
    return import_Promises.MaybePromise.then(
      this.moveToHueLogic(targetHue, import_ColorControlCluster.ColorControl.Direction.ShortestDistance, transitionTime, false),
  ...
  moveToColor({
    ...
    return import_Promises.MaybePromise.then(
      this.setColorMode(import_ColorControlCluster.ColorControl.ColorMode.CurrentXAndCurrentY),
  ...
  moveToColorLogic(targetX, targetY, transitionTime) {
    if (!this.state.managedTransitionTimeHandling || transitionTime === 0) {
      ...
      this.state.currentX = (0, import_Number.cropValueRange)(targetX, MIN_CIE_XY_VALUE, MAX_CIE_XY_VALUE);
      this.state.currentY = (0, import_Number.cropValueRange)(targetY, MIN_CIE_XY_VALUE, MAX_CIE_XY_VALUE);
      this.state.remainingTime = 0;
      return;
    }
  ...
  stepColorLogic(stepX, stepY, transitionTime) {
    const targetValueX = (0, import_Number.cropValueRange)(this.state.currentX + stepX, MIN_CIE_XY_VALUE, MAX_CIE_XY_VALUE);
    const targetValueY = (0, import_Number.cropValueRange)(this.state.currentY + stepY, MIN_CIE_XY_VALUE, MAX_CIE_XY_VALUE);
    if (!this.state.managedTransitionTimeHandling || transitionTime === 0) {
      this.internal.stopXTransition();
      this.internal.stopYTransition();
  ...
  moveToColorTemperature({
    ...
    return import_Promises.MaybePromise.then(
      this.setColorMode(import_ColorControlCluster.ColorControl.ColorMode.ColorTemperatureMireds),
  ...
  enhancedMoveToHue({
    ...
    return import_Promises.MaybePromise.then(
      this.setEnhancedColorMode(import_ColorControlCluster.ColorControl.EnhancedColorMode.EnhancedCurrentHueAndCurrentSaturation),
  ...
  moveToEnhancedHueAndSaturationLogic(targetEnhancedHue, targetSaturation, transitionTime) {
    return import_Promises.MaybePromise.then(
      this.moveToHueLogic(targetEnhancedHue, import_ColorControlCluster.ColorControl.Direction.ShortestDistance, transitionTime, true),
      () => this.moveToSaturationLogic(targetSaturation, transitionTime)
    );
  }
  /**
  ...
  colorLoopSet({
    ...
    if (updateFlags.updateAction) {
      if (action === import_ColorControlCluster.ColorControl.Action.DeActivateTheColorLoop) {
        ...
        return import_Promises.MaybePromise.then(
          ...
          () => {
            ...
            this.state.colorLoopActive = import_ColorControlCluster.ColorControl.ColorLoopActive.Active;
            if (action === import_ColorControlCluster.ColorControl.Action.ActivateTheColorLoopFromTheValueInTheColorLoopStartEnhancedHueField) {
              return this.startColorLoopLogic(this.state.colorLoopStartEnhancedHue);
            } else if (action === import_ColorControlCluster.ColorControl.Action.ActivateTheColorLoopFromTheValueOfTheEnhancedCurrentHueAttribute) {
   ...
   */
  #stopColorLoop() {
    return import_Promises.MaybePromise.then(this.stopColorLoopLogic(), () => {
      if (this.state.colorLoopActive === import_ColorControlCluster.ColorControl.ColorLoopActive.Active) {
        this.state.colorLoopActive = import_ColorControlCluster.ColorControl.ColorLoopActive.Inactive;
  ...
  startColorLoopLogic(startHue) {
    ...
    return this.moveHueLogic(
      this.state.colorLoopDirection === import_ColorControlCluster.ColorControl.ColorLoopDirection.Decrement ? import_ColorControlCluster.ColorControl.MoveMode.Down : import_ColorControlCluster.ColorControl.MoveMode.Up,
   ...
   */
  stopMoveStepLogic() {
    if (this.state.colorLoopActive === import_ColorControlCluster.ColorControl.ColorLoopActive.Inactive) {
      this.internal.stopHueTransition();
    }
    this.internal.stopSaturationTransition();
    this.internal.stopColorTemperatureTransition();
  ...
  moveColorTemperature({
    ...
    return import_Promises.MaybePromise.then(this.setColorMode(import_ColorControlCluster.ColorControl.ColorMode.ColorTemperatureMireds), () => {
      if (moveMode === import_ColorControlCluster.ColorControl.MoveMode.Stop) {
        return this.stopHueAndSaturationMovement();
      }
      ...
      colorTemperatureMinimumMireds = (0, import_Number.cropValueRange)(
        colorTemperatureMinimumMireds,
        this.minimumColorTemperatureMireds,
        this.maximumColorTemperatureMireds
      );
      colorTemperatureMaximumMireds = (0, import_Number.cropValueRange)(
        colorTemperatureMaximumMireds,
        this.minimumColorTemperatureMireds,
        this.maximumColorTemperatureMireds
      );
  ...
  moveColorTemperatureLogic(moveMode, rate, colorTemperatureMinimumMireds, colorTemperatureMaximumMireds) {
    if (!this.state.managedTransitionTimeHandling) {
      this.internal.stopColorTemperatureTransition();
      this.state.colorTemperatureMireds = moveMode === import_ColorControlCluster.ColorControl.MoveMode.Up ? colorTemperatureMaximumMireds : colorTemperatureMinimumMireds;
      this.state.remainingTime = 0;
      return;
    }
    const effectiveRate = rate * (moveMode === import_ColorControlCluster.ColorControl.MoveMode.Up ? 1 : -1);
    this.#initiateTransition(
      "colorTemperature",
      effectiveRate,
      colorTemperatureMinimumMireds,
  ...
  stepColorTemperatureLogic(stepMode, stepSize, transitionTime, colorTemperatureMinimumMireds, colorTemperatureMaximumMireds) {
    const targetValue = (0, import_Number.cropValueRange)(
      this.state.colorTemperatureMireds + (stepMode === import_ColorControlCluster.ColorControl.StepMode.Up ? stepSize : -stepSize),
      colorTemperatureMinimumMireds,
      colorTemperatureMaximumMireds
    );
    if (!this.state.managedTransitionTimeHandling || transitionTime === 0) {
  ...
  setColorMode(mode) {
    ...
    return import_Promises.MaybePromise.then(this.switchColorMode(this.state.colorMode, mode), () => {
  ...
  setEnhancedColorMode(mode) {
    ...
    let colorModeSet;
    if (mode === import_ColorControlCluster.ColorControl.EnhancedColorMode.EnhancedCurrentHueAndCurrentSaturation) {
      colorModeSet = this.setColorMode(import_ColorControlCluster.ColorControl.ColorMode.CurrentHueAndCurrentSaturation);
    } else {
      colorModeSet = this.setColorMode(mode);
    }
    return import_Promises.MaybePromise.then(colorModeSet, () => {
      this.state.enhancedColorMode = mode;
    });
  }
  /**
  ...
  switchColorMode(oldMode, newMode) {
    ...
    switch (oldMode) {
      case import_ColorControlCluster.ColorControl.ColorMode.CurrentHueAndCurrentSaturation:
        switch (newMode) {
          case import_ColorControlCluster.ColorControl.ColorMode.CurrentXAndCurrentY:
            const [x, y] = (0, import_ColorConversionUtils.hsvToXy)(this.hue, this.saturation);
            this.x = x;
            this.y = y;
            break;
          case import_ColorControlCluster.ColorControl.ColorMode.ColorTemperatureMireds:
  ...
  syncColorTemperatureWithLevel(level) {
    if (this.state.colorMode === import_ColorControlCluster.ColorControl.ColorMode.ColorTemperatureMireds || this.state.enhancedColorMode === import_ColorControlCluster.ColorControl.EnhancedColorMode.ColorTemperatureMireds) {
  ...
  #assertRate(mode, rate) {
    if ((mode === import_ColorControlCluster.ColorControl.MoveMode.Up || mode === import_ColorControlCluster.ColorControl.MoveMode.Down) && rate === 0) {
      throw new import_StatusCode.StatusResponseError("Rate must not be 0 when moving Up or Down", import_StatusCode.StatusCode.InvalidCommand);
    }
  }
  ...
  #getHueDistanceByDirection(currentHue, targetHue, direction, max) {
    ...
    if (direction === import_ColorControlCluster.ColorControl.Direction.Up) {
      return distance;
    } else if (direction === import_ColorControlCluster.ColorControl.Direction.Down) {
      return -(max - distance);
    }
    if (direction === import_ColorControlCluster.ColorControl.Direction.ShortestDistance) {
      if (Math.abs(distance) > max / 2) {
        return -(max - distance);
      }
      return distance;
    }
    if (direction === import_ColorControlCluster.ColorControl.Direction.LongestDistance) {
      if (Math.abs(distance) > max / 2) {
        return distance;
      }
      return -(max - distance);
    }
    throw new import_MatterError.ImplementationError(`Unknown direction: ${direction}`);
  }
  ...
  #initializeManagedTransitionTimers() {
    ...
    this.internal.hueTransitionIntervalTimer = import_Time.Time.getPeriodicTimer(
      "ColorControl.hue.transition",
      1e3,
      this.callback(this.#hueStepIntervalTick)
    );
    this.internal.saturationTransitionIntervalTimer = import_Time.Time.getPeriodicTimer(
      "ColorControl.saturation.transition",
      1e3,
      this.callback(this.#saturationStepIntervalTick)
    );
    this.internal.colorTemperatureTransitionIntervalTimer = import_Time.Time.getPeriodicTimer(
      "ColorControl.colorTemperature.transition",
      1e3,
      this.callback(this.#colorTemperatureStepIntervalTick)
    );
    this.internal.xTransitionIntervalTimer = import_Time.Time.getPeriodicTimer(
      "ColorControl.x.transition",
      1e3,
      this.callback(this.#xStepIntervalTick)
    );
    this.internal.yTransitionIntervalTimer = import_Time.Time.getPeriodicTimer(
  ...
  #getBootReason() {
    const rootEndpoint = this.endpoint.ownerOfType(import_RootEndpoint.RootEndpoint);
    if (rootEndpoint !== void 0 && rootEndpoint.behaviors.has(import_GeneralDiagnosticsBehavior.GeneralDiagnosticsBehavior)) {
      return rootEndpoint.stateOf(import_GeneralDiagnosticsBehavior.GeneralDiagnosticsBehavior).bootReason;
    }
  }
  ...
  #optionsAllowExecution(optionsMask, optionsOverride) {
    ...
    return options.executeIfOff || !this.agent.has(import_OnOffServer.OnOffServer) || this.agent.get(import_OnOffServer.OnOffServer).state.onOff;
  }
  #initiateTransition(transitionType, changeRate, minValue, maxValue, targetValue) {
    ...
    if (transitionType === "enhancedHue" || transitionType === "hue") {
      if (targetValue !== void 0) {
        const direction = changeRate > 0 ? import_ColorControlCluster.ColorControl.Direction.Up : import_ColorControlCluster.ColorControl.Direction.Down;
  ...
  #stepIntervalTick(transitionType, transitionTimer, transitionData, currentValue) {
    ...
    switch (transitionType) {
      ...
      case "enhancedHue":
        ...
        newValue = (0, import_Number.addValueWithOverflow)(
          currentValue,
          changeRate,
        ...
        break;
      case "saturation":
        newValue = (0, import_Number.cropValueRange)(
      ...
      case "y":
        ...
        break;
      default:
        throw new import_MatterError.ImplementationError(`Unknown transition type: ${transitionType}`);
    }
  ...
  #setFromXyValue(fieldName, value) {
    if (value === void 0) {
      if (this.state[fieldName] === void 0) {
        ...
        throw new import_MatterError.ImplementationError(`${fieldName} cannot be set to undefined while being enabled.`);
      }
    }
    this.state[fieldName] = (0, import_Number.cropValueRange)(Math.round(value * 65536), MIN_CIE_XY_VALUE, MAX_CIE_XY_VALUE);
  }
...
class ColorControlServer extends ColorControlServerLogic.for((0, import_ClusterType.ClusterType)(import_ColorControlCluster.ColorControl.Base)) {
}
