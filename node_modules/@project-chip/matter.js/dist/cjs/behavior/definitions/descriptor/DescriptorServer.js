...
var import_IndexBehavior = require("../../system/index/IndexBehavior.js");
...
class DescriptorServer extends import_DescriptorBehavior.DescriptorBehavior {
  ...
  addDeviceTypes(...deviceTypes) {
    ...
    nextInput: for (let newDeviceType of deviceTypes) {
      if (typeof newDeviceType === "string") {
        ...
        if (dt === void 0) {
          throw new import_MatterError.ImplementationError(`Device type ${newDeviceType} not found`);
        }
        newDeviceType = { deviceType: (0, import_DeviceTypeId.DeviceTypeId)(dt.id), revision: dt.revision };
      }
      for (const existingDeviceType of list) {
        if ((0, import_DeepEqual.isDeepEqual)(newDeviceType, existingDeviceType)) {
          continue nextInput;
        }
      }
  ...
  addTags(...tags) {
    if (!this.features.tagList) {
      throw new import_MatterError.ImplementationError('You must enable the descriptor "TagList" feature to set tags');
    }
