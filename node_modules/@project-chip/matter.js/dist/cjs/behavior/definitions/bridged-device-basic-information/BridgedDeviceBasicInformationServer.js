...
var import_AggregatorEndpoint = require("../../../endpoint/definitions/system/AggregatorEndpoint.js");
var import_Logger = require("../../../log/Logger.js");
var import_DescriptorServer = require("../descriptor/DescriptorServer.js");
 ...
 */
const logger = import_Logger.Logger.get("BridgedDeviceBasicInformationServer");
class BridgedDeviceBasicInformationServer extends import_BridgedDeviceBasicInformationBehavior.BridgedDeviceBasicInformationBehavior {
  async initialize() {
    if (this.endpoint.lifecycle.isInstalled) {
      await this.agent.load(import_DescriptorServer.DescriptorServer);
  ...
  async #configurePart() {
    ...
    if (owner === void 0) {
      throw new import_MatterError.ImplementationError(`Bridged node ${this.endpoint} has no parent`);
    }
    if (!owner.get(import_DescriptorServer.DescriptorServer).hasDeviceType(import_AggregatorEndpoint.AggregatorEndpoint.deviceType)) {
      throw new import_MatterError.ImplementationError(`Bridged node ${this.endpoint} owner ${owner} is not an aggregator`);
    }
    this.agent.get(import_DescriptorServer.DescriptorServer).addDeviceTypes("BridgedNode");
  }
}
