...
var import_ColorControlServer = require("../color-control/ColorControlServer.js");
 ...
 */
const logger = import_Logger.Logger.get("LevelControlServer");
const LevelControlLogicBase = import_LevelControlBehavior.LevelControlBehavior.with(import_LevelControlCluster.LevelControl.Feature.OnOff, import_LevelControlCluster.LevelControl.Feature.Lighting);
class LevelControlServerLogic extends LevelControlLogicBase {
  ...
  initialize() {
    if (this.state.managedTransitionTimeHandling) {
      this.internal.transitionIntervalTimer = import_Time.Time.getPeriodicTimer(
    ...
    if (this.features.lighting) {
      ...
      if (this.#getBootReason() !== import_definitions.GeneralDiagnostics.BootReason.SoftwareUpdateCompleted) {
    ...
    if (this.features.onOff && this.agent.has(import_OnOffServer.OnOffServer)) {
      this.reactTo(this.agent.get(import_OnOffServer.OnOffServer).events.onOff$Changed, this.handleOnOffChange);
    }
  }
  /**
  ...
  moveToLevelLogic(level, transitionTime, withOnOff, options = {}) {
    ...
    level = (0, import_Number.cropValueRange)(level, this.minLevel, this.maxLevel);
  ...
  moveLogic(moveMode, rate, withOnOff, options = {}) {
    ...
    if (!this.state.managedTransitionTimeHandling || effectiveRate === null || effectiveRate === 0) {
      const level = effectiveRate === 0 ? this.currentLevel : moveMode === import_LevelControlCluster.LevelControl.MoveMode.Up ? this.maxLevel : this.minLevel;
      this.setRemainingTime(0);
      return this.setLevel(level, withOnOff, options);
    }
    return this.#initiateTransition(
      effectiveRate * (moveMode === import_LevelControlCluster.LevelControl.MoveMode.Up ? 1 : -1),
      withOnOff,
      void 0,
      options
    );
  ...
  stepLogic(stepMode, stepSize, transitionTime, withOnOff, options = {}) {
    const targetLevel = (0, import_Number.cropValueRange)(
      stepMode === import_LevelControlCluster.LevelControl.StepMode.Up ? this.currentLevel + stepSize : this.currentLevel - stepSize,
      this.minLevel,
      this.maxLevel
    );
    ...
    const effectiveRate = Math.ceil(stepSize / transitionTime * 10) * (stepMode === import_LevelControlCluster.LevelControl.StepMode.Up ? 1 : -1);
  ...
  setLevel(level, withOnOff, options = {}) {
    const onOffServer = this.features.onOff && withOnOff && this.agent.has(import_OnOffServer.OnOffServer) ? this.agent.get(import_OnOffServer.OnOffServer) : void 0;
    if (onOffServer !== void 0 && level === this.minLevel && onOffServer.state.onOff) {
      const offPromise = onOffServer.off();
      return import_Promises.MaybePromise.then(offPromise, () => {
        this.state.currentLevel = level;
      });
    }
    this.state.currentLevel = level;
    let colorSyncResult;
    if (this.features.lighting && options.coupleColorTempToLevel && this.agent.has(import_ColorControlServer.ColorControlServer)) {
      colorSyncResult = this.agent.get(import_ColorControlServer.ColorControlServer).syncColorTemperatureWithLevel(level);
    }
    if (onOffServer !== void 0 && level > this.minLevel && !onOffServer.state.onOff) {
      return import_Promises.MaybePromise.then(colorSyncResult, () => onOffServer.on());
    } else {
  ...
  #optionsAllowExecution(options) {
    return options.executeIfOff || !this.features.onOff || !this.agent.has(import_OnOffServer.OnOffServer) || this.agent.get(import_OnOffServer.OnOffServer).state.onOff;
  }
...
class LevelControlServer extends LevelControlServerLogic.with(import_LevelControlCluster.LevelControl.Feature.OnOff) {
}
