...
var import_WindowCoveringCluster = require("../../../cluster/definitions/WindowCoveringCluster.js");
 ...
 */
const logger = import_Logger.Logger.get("WindowCoveringServer");
const WindowCoveringServerBase = import_WindowCoveringBehavior.WindowCoveringBehavior.with(
  import_WindowCoveringCluster.WindowCovering.Feature.Lift,
  import_WindowCoveringCluster.WindowCovering.Feature.Tilt,
  import_WindowCoveringCluster.WindowCovering.Feature.PositionAwareLift,
  import_WindowCoveringCluster.WindowCovering.Feature.PositionAwareTilt,
  import_WindowCoveringCluster.WindowCovering.Feature.AbsolutePosition
);
...
class WindowCoveringServerLogic extends WindowCoveringServerBase {
  initialize() {
    ...
    if (!(0, import_DeepEqual.isDeepEqual)(configStatus, this.state.configStatus)) {
  ...
  #handleModeChanging(mode) {
    ...
    if (mode.maintenanceMode && !this.state.supportsMaintenanceMode) {
      throw new import_StatusCode.StatusResponseError("Maintenance mode not supported", import_StatusCode.StatusCode.ConstraintError);
    }
    this.internal.inMaintenanceMode = !!mode.maintenanceMode;
    if (mode.calibrationMode) {
      if (!this.state.supportsCalibration) {
        throw new import_StatusCode.StatusResponseError("Calibration not supported", import_StatusCode.StatusCode.ConstraintError);
      }
    ...
    logger.debug(
      `Mode changed to ${import_Logger.Logger.toJSON(mode)} and config status to ${import_Logger.Logger.toJSON(configStatus)} and internal calibration mode to ${this.internal.calibrationMode}`
    );
  }
  /** Update the global operational status based on the lift or tilt status. */
  #handleOperationalStatusChanging(operationalStatus) {
    const globalStatus = operationalStatus.lift !== import_WindowCoveringCluster.WindowCovering.MovementStatus.Stopped ? operationalStatus.lift : operationalStatus.tilt;
    operationalStatus.global = globalStatus;
    logger.debug(
      `Operational status changed to ${import_Logger.Logger.toJSON(operationalStatus)} with new global status ${globalStatus}`
    );
  ...
  #syncLiftCurrentPositions(percent100ths) {
    if (this.features.positionAwareLift) {
      ...
      if (this.state.operationalStatus.lift !== import_WindowCoveringCluster.WindowCovering.MovementStatus.Stopped && percent100ths === this.state.targetPositionLiftPercent100ths) {
        this.state.operationalStatus.lift = import_WindowCoveringCluster.WindowCovering.MovementStatus.Stopped;
  ...
  #syncTiltCurrentPositions(percent100ths) {
    if (this.features.positionAwareTilt) {
      ...
      if (this.state.operationalStatus.tilt !== import_WindowCoveringCluster.WindowCovering.MovementStatus.Stopped && percent100ths === this.state.targetPositionTiltPercent100ths) {
        this.state.operationalStatus.tilt = import_WindowCoveringCluster.WindowCovering.MovementStatus.Stopped;
  ...
  #computeOperationalState(target, current) {
    if (current === null || target === null) {
      return import_WindowCoveringCluster.WindowCovering.MovementStatus.Stopped;
    } else if (current === target) {
      return import_WindowCoveringCluster.WindowCovering.MovementStatus.Stopped;
    } else if (current < target) {
      return import_WindowCoveringCluster.WindowCovering.MovementStatus.Closing;
    } else {
      return import_WindowCoveringCluster.WindowCovering.MovementStatus.Opening;
    }
  }
  /**
  ...
  #assertMotionLockStatus() {
    if (this.internal.inMaintenanceMode) {
      throw new import_StatusCode.StatusResponseError("Device is in maintenance mode", import_StatusCode.StatusCode.Busy);
    }
    switch (this.internal.calibrationMode) {
      case 0 /* Enabled */:
        if (!this.state.supportsCalibration) {
          throw new import_StatusCode.StatusResponseError("Calibration not implemented", import_StatusCode.StatusCode.Failure);
        }
        break;
      case 1 /* Running */:
        break;
    ...
    if (!this.state.configStatus.operational) {
      throw new import_StatusCode.StatusResponseError("Device is not operational", import_StatusCode.StatusCode.Failure);
    }
  }
  /**
      ...
      case 0 /* Lift */:
        if (this.features.positionAwareLift) {
          if (targetPercent100ths === void 0) {
            throw new import_MatterError.ImplementationError("Target position must be defined for position aware lift");
          }
  ...
  #prepareMovement(type, direction, targetPercent100ths) {
    if (this.state.supportsCalibration && this.internal.calibrationMode === 0 /* Enabled */) {
    ...
    switch (type) {
      case 0 /* Lift */:
        ...
        if (direction !== 2 /* DefinedByPosition */) {
          this.state.operationalStatus.lift = direction === 1 /* Close */ ? import_WindowCoveringCluster.WindowCovering.MovementStatus.Closing : import_WindowCoveringCluster.WindowCovering.MovementStatus.Opening;
        }
        break;
      case 1 /* Tilt */:
        ...
        if (direction !== 2 /* DefinedByPosition */) {
          this.state.operationalStatus.tilt = direction === 1 /* Close */ ? import_WindowCoveringCluster.WindowCovering.MovementStatus.Closing : import_WindowCoveringCluster.WindowCovering.MovementStatus.Opening;
        }
        break;
    }
  ...
  #executeCalibrationAndMove(type, direction, targetPercent100ths) {
    let calibration;
    if (this.internal.calibrationMode === 0 /* Enabled */ && this.state.supportsCalibration) {
      this.internal.calibrationMode = 1 /* Running */;
      calibration = this.executeCalibration();
    }
    return import_Promises.MaybePromise.then(calibration, () => {
      this.internal.calibrationMode = 2 /* Disabled */;
  ...
  handleStopMovement() {
    ...
    if (!this.features.positionAwareLift && !this.features.positionAwareTilt) {
      this.state.operationalStatus = {
        global: import_WindowCoveringCluster.WindowCovering.MovementStatus.Stopped,
        lift: import_WindowCoveringCluster.WindowCovering.MovementStatus.Stopped,
        tilt: import_WindowCoveringCluster.WindowCovering.MovementStatus.Stopped
      };
    }
  }
...
class WindowCoveringServer extends WindowCoveringServerLogic.for((0, import_ClusterType.ClusterType)(import_WindowCoveringCluster.WindowCovering.Base)) {
}
