...
function createType(cluster, base, schema) {
  ...
  schema.freeze();
  return (0, import_GeneratedClass.GeneratedClass)({
    name,
    base,
    ...
    staticDescriptors: {
      id: {
        value: (0, import_String.camelize)(cluster.name),
        enumerable: true
      },
      cluster: {
        value: cluster,
...
function createDerivedState(cluster, schema, base, namesUsed) {
  ...
  if (BaseState === void 0) {
    throw new import_MatterError.ImplementationError(`No state class defined for behavior class ${base.name}`);
  }
  ...
  for (const name in props) {
    ...
    if (attribute?.id === import_FeatureMap.FeatureMap.id) {
  ...
  return (0, import_GeneratedClass.GeneratedClass)({
    ...
    initialize() {
      for (const name of names) {
        this[name] = (0, import_Observable.AsyncObservable)();
      }
    }
  });
}
function schemaForCluster(cluster) {
  ...
  for (const child of import_model.Matter.children) {
    if (child.tag === import_model.ElementTag.Cluster && child.id === cluster.id) {
      schema = new import_model.ClusterModel(child);
      break;
    }
  }
  if (schema === void 0) {
    throw new import_MatterError.ImplementationError(`Cannot locate schema for cluster ${cluster.id}, please supply manually`);
  }
  return schema;
}
...
function selectDefaultValue(oldDefault, clusterAttr, schemaProp) {
