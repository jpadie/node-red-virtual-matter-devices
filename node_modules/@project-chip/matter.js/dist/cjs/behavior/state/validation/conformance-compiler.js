 ...
 */
function astToFunction(schema, featureMap, supportedFeatures) {
  const ast = schema.conformance.ast;
  const { featuresAvailable, featuresSupported } = import_model.FeatureSet.normalize(featureMap, supportedFeatures);
  ...
  function createName(param) {
    if (featuresAvailable.has(param)) {
      ...
      param = (0, import_String.camelize)(param);
      return {
        code: import_conformance_util.Code.Evaluate,
