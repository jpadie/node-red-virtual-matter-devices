...
function createConfig(owner, schema) {
  ...
  if (entry === void 0) {
    throw new import_errors.SchemaImplementationError((0, import_DataModelPath.DataModelPath)(schema.path), "List schema has no entry definition");
  }
    ...
    readEntry = (index) => {
      ...
      if (subref === void 0) {
        subref = (0, import_ManagedReference.ManagedReference)(
          ...
          index,
          () => true,
          (val) => Array.isArray(val) ? [...val] : (0, import_Type.isObject)(val) ? { ...val } : val
        );
  ...
  if (config.fabricScoped) {
    ...
    if (session.fabricFiltered || config.fabricSensitive) {
      ...
      writeEntry = (index, value, location) => {
        if (value === void 0) {
          ...
          if (typeof value !== "object") {
            throw new import_errors.WriteError(location, `Fabric scoped list value is not an object`, import_StatusCode.StatusCode.Failure);
          }
      ...
      getListLength = () => {
        ...
        for (let i = 0; i < readVal().length; i++) {
          const entry = readVal()[i];
          if ((0, import_Type.isObject)(entry) && (session.offline || !entry.fabricIndex || entry.fabricIndex === session.fabric)) {
            length++;
          }
        }
        return length;
        ...
        return {
          next() {
            while (true) {
              ...
              if (!next.done && (0, import_Type.isObject)(next.value) && next.value.fabricIndex !== session.fabric) {
                continue;
              }
              return next;
            }
  ...
  const handlers = {
    get(_target, property, receiver) {
      ...
      switch (property) {
        ...
        case "toString":
          return function() {
            return (0, import_String.serialize)(this);
          };
