...
function getDefaults(schema) {
  ...
  for (const member of schema.activeMembers) {
    if (member.default !== void 0) {
      defaults[(0, import_String.camelize)(member.name)] = member.default;
      continue;
    }
    if (member.mandatory && member.nullable) {
      defaults[(0, import_String.camelize)(member.name)] = null;
      continue;
    }
  }
...
function StructPatcher(schema, owner) {
  ...
  for (const member of schema.activeMembers) {
    ...
    const key = (0, import_String.camelize)(member.name);
    ...
    for (const indexStr in changes) {
      ...
      if (patchEntry) {
        ...
        if (newValue === void 0 || newValue === null || oldValue === void 0 || oldValue === null) {
          if (entryDefaults && (0, import_Type.isObject)(newValue)) {
...
function PrimitivePatcher() {
  return (_changes, _target, path) => {
    throw new import_MatterError.ImplementationError(`Cannot generate patch ${path} because it does not define not a collection`);
  };
}
