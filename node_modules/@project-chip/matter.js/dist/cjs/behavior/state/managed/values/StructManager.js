...
function StructManager(owner, schema) {
  ...
  for (const member of schema.activeMembers) {
    const name2 = (0, import_String.camelize)(member.name);
    ...
    instanceDescriptors: {
      ...
      [AUTHORIZE_READ]: {
        value(index) {
          ...
          if (access === void 0) {
            throw new import_MatterError.ImplementationError(`Direct read of unknown property ${index}`);
          }
...
((StructManager2) => {
  function assertDirectReadAuthorized(struct, index) {
    if (!struct?.[AUTHORIZE_READ]) {
      throw new import_MatterError.ImplementationError("Cannot authorize read of unmanaged value");
    }
...
function configureProperty(manager, schema) {
  const name = (0, import_String.camelize)(schema.name);
