...
var import_GeneratedClass = require("../../../../util/GeneratedClass.js");
...
function BitmapManager(_owner, schema) {
  const instanceDescriptors = {};
  const byteSize = schema.metabase?.byteSize;
  if (byteSize === void 0) {
    throw new import_errors.SchemaImplementationError((0, import_DataModelPath.DataModelPath)(schema.path), `Base bitmap type has no byteSize defined`);
  }
  ...
  for (const member of schema.activeMembers) {
    ...
    let name;
    if (schema.id === import_FeatureMap.FeatureMap.id) {
      name = (0, import_String.camelize)(member.description ?? member.name);
    } else {
      name = (0, import_String.camelize)(member.name);
    }
    const descriptor = configureProperty(name, maxBit, member);
    instanceDescriptors[name] = descriptor;
  }
  const Wrapper = (0, import_GeneratedClass.GeneratedClass)({
    name: schema.name,
    initialize(ref, session) {
      if (!(0, import_Type.isObject)(ref.value)) {
...
function configureProperty(name, maxBit, schema) {
  ...
  if (typeof constraint.value === "number") {
    ...
    throw new import_errors.SchemaImplementationError((0, import_DataModelPath.DataModelPath)(schema.path), `Bitfield is not properly constrained`);
  }
  if (stopBit > maxBit) {
    throw new import_errors.SchemaImplementationError(
