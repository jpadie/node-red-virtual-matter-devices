 ...
 */
const logger = import_Logger.Logger.get("BehaviorBacking");
class BehaviorBacking {
   ...
   */
  [import_Construction.Construction.construct](agent) {
    const crash = (cause) => {
  ...
  close(agent, invokeClose = true) {
    const initialized = this.construction.status === import_Lifecycle.Lifecycle.Status.Active;
    if (!initialized) {
      invokeClose = false;
    }
    return this.construction.close(() => {
      let result = import_Promises.MaybePromise.then(
        () => this.#reactors?.close(),
        () => {
      ...
      if (invokeClose) {
        result = import_Promises.MaybePromise.then(result, () => this.#invokeClose(agent));
      }
      return result;
    });
  }
  ...
  #invokeClose(agent) {
    const behavior = this.#lifecycleInstance(agent);
    return import_Promises.MaybePromise.finally(
      () => import_Promises.MaybePromise.then(
