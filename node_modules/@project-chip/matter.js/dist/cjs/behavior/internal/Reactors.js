 ...
 */
const logger = import_Logger.Logger.get("Reactors");
class Reactors {
  #backing;
...
class ReactorBacking {
  ...
  constructor(reactors, observable, reactor, { offline, once, lock }) {
    ...
    const reactorListener = (...args) => {
      ...
      if (!alreadyReacting) {
        if (import_Promises.MaybePromise.is(resolution)) {
      ...
      } else if (import_Promises.MaybePromise.is(resolution)) {
  ...
  #react(args) {
    ...
    if (context) {
      ...
      if (import_Promises.MaybePromise.is(result) && !this.#observable.isAsync) {
  ...
  async #lockThenReact(context, backing, args) {
    if (!this.#lock) {
      throw new import_MatterError.InternalError("No locks to acquire");
    }
    ...
    for (const resource of context.transaction.resources) {
      if (resource.lockedBy !== context.transaction) {
        throw new import_MatterError.InternalError(`Lock of ${resource} should be held by ${this} but is not`);
      }
    }
