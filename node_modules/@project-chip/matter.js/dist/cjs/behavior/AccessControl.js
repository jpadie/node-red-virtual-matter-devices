...
var import_aspects = require("../model/aspects/index.js");
var import_model = require("../model/index.js");
var import_models = require("../model/models/index.js");
...
function dataEnforcerFor(schema) {
  ...
  let authorizeRead = (session, location) => {
    ...
    throw new import_errors.ReadError(location, "Permission denied", import_StatusCode.StatusCode.UnsupportedAccess);
  };
  let authorizeWrite = (session, location) => {
    ...
    throw new import_errors.WriteError(location, "Permission denied", import_StatusCode.StatusCode.UnsupportedAccess);
  };
  if (limits.timed) {
  ...
  if (limits.fabricSensitive) {
    ...
    authorizeWrite = (session, location) => {
      ...
      if (session.fabric === void 0) {
        throw new import_errors.WriteError(location, "Permission denied: No accessing fabric", import_StatusCode.StatusCode.UnsupportedAccess);
      }
      if (location?.owningFabric && location.owningFabric !== session.fabric) {
...
function commandEnforcerFor(schema) {
  ...
  return {
    ...
    authorizeInvoke(session, location) {
      ...
      throw new import_errors.InvokeError(location, "Permission denied", import_StatusCode.StatusCode.UnsupportedAccess);
    },
...
function limitsFor(schema) {
  ...
  const quality = schema instanceof import_models.ValueModel ? schema.effectiveQuality : void 0;
  let fixed = quality?.fixed;
  for (let s = schema.parent; !fixed && s instanceof import_models.ValueModel; s = s.parent) {
  ...
  const limits = Object.freeze({
    ...
    fabricScoped: access.fabric === import_aspects.Access.Fabric.Scoped || access.fabric === import_aspects.Access.Fabric.Sensitive,
    fabricSensitive: access.fabric === import_aspects.Access.Fabric.Sensitive,
    ...
    readLevel: access.readPriv === void 0 ? import_Cluster.AccessLevel.View : import_aspects.Access.PrivilegeLevel[access.readPriv],
    writeLevel: access.writePriv === void 0 ? import_Cluster.AccessLevel.Operate : import_aspects.Access.PrivilegeLevel[access.writePriv]
  });
  return limits;
}
