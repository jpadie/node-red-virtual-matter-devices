 ...
 */
const logger = import_Logger.Logger.get("Fabric");
const COMPRESSED_FABRIC_ID_INFO = import_ByteArray.ByteArray.fromString("CompressedFabric");
const GROUP_SECURITY_INFO = import_ByteArray.ByteArray.fromString("GroupKey v1.0");
class PublicKeyError extends import_MatterError.MatterError {
}
...
class Fabric {
  ...
  static createFromStorageObject(fabricObject) {
    return new Fabric(
      ...
      (0, import_Key.PrivateKey)(fabricObject.keyPair),
  ...
  sign(data) {
    return import_Crypto.Crypto.sign(this.#keyPair, data);
  }
  ...
  getDestinationId(nodeId, random) {
    ...
    return import_Crypto.Crypto.hmac(this.operationalIdentityProtectionKey, writer.toByteArray());
  }
  addSession(session) {
  ...
  getGroupSetForIpk() {
    return {
      ...
      groupKeySecurityPolicy: import_GroupKeyManagementCluster.GroupKeyManagement.GroupKeySecurityPolicy.TrustFirst,
      groupKeyMulticastPolicy: import_GroupKeyManagementCluster.GroupKeyManagement.GroupKeyMulticastPolicy.PerGroupId
    };
  }
  getAllGroupKeySets() {
    return [OperationalGroupKeySet.asTlvGroupSet(this.getGroupSetForIpk())];
...
class FabricBuilder {
  #keyPair = import_Crypto.Crypto.createKeyPair();
  #rootVendorId;
  #rootCert;
  #intermediateCACert;
  #operationalCert;
  ...
  setOperationalCert(operationalCert, intermediateCACert) {
    ...
    if (this.#rootCert === void 0) {
      throw new import_MatterError.MatterFlowError("Root Certificate needs to be set first.");
    }
    if (intermediateCACert !== void 0) {
    ...
    this.#operationalCert = operationalCert;
    this.#intermediateCACert = intermediateCACert;
    this.#fabricId = (0, import_FabricId.FabricId)(fabricId);
    this.#nodeId = nodeId;
    return this;
  }
  ...
  matchesToFabric(fabric) {
    if (this.#fabricId === void 0 || this.#rootPublicKey === void 0) {
      throw new import_MatterError.MatterFlowError("Node Operational Data needs to be set first.");
    }
  ...
  async build(fabricIndex) {
    if (this.#fabricIndex !== void 0) throw new import_MatterError.InternalError("FabricBuilder can only be built once");
    if (this.#rootNodeId === void 0) throw new import_MatterError.InternalError("rootNodeId needs to be set");
    if (this.#rootVendorId === void 0) throw new import_MatterError.InternalError("vendorId needs to be set");
    if (this.#rootCert === void 0 || this.#rootPublicKey === void 0)
      throw new import_MatterError.InternalError("rootCert needs to be set");
    if (this.#identityProtectionKey === void 0) throw new import_MatterError.InternalError("identityProtectionKey needs to be set");
    if (this.#operationalCert === void 0 || this.#fabricId === void 0 || this.#nodeId === void 0)
      throw new import_MatterError.InternalError("operationalCert needs to be set");
    this.#fabricIndex = fabricIndex;
    const saltWriter = new import_DataWriter.DataWriter(import_ByteArray.Endian.Big);
    saltWriter.writeUInt64(this.#fabricId);
    const operationalId = await import_Crypto.Crypto.hkdf(
    ...
    return new Fabric(
      ...
      // Epoch Key
      await import_Crypto.Crypto.hkdf(this.#identityProtectionKey, operationalId, GROUP_SECURITY_INFO),
