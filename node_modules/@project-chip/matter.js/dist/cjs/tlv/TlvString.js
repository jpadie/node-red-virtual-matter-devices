...
class StringSchema extends import_TlvSchema.TlvSchema {
  constructor(type, minLength = 0, maxLength = 1024) {
    ...
    if (minLength < 0) throw new import_MatterError.InternalError("Minimum length should be a positive number.");
  }
  ...
  decodeTlvInternalValue(reader, typeLength) {
    if (typeLength.type !== this.type) throw new import_MatterError.UnexpectedDataError(`Unexpected type ${typeLength.type}.`);
  ...
  validate(value) {
    ...
    if (this.type === import_TlvCodec.TlvType.ByteString && !(value instanceof import_ByteArray.ByteArray))
      throw new import_ValidationError.ValidationDatatypeMismatchError(`Expected ByteArray, got ${typeof value}.`);
    if (value.length > this.maxLength)
      throw new import_ValidationError.ValidationOutOfBoundsError(
        `String ${(0, import_String.serialize)(value)} is too long: ${value.length}, max ${this.maxLength}.`
      );
    if (value.length < this.minLength)
      throw new import_ValidationError.ValidationOutOfBoundsError(
        `String ${(0, import_String.serialize)(value)} is too short: ${value.length}, min ${this.minLength}.`
      );
  }
