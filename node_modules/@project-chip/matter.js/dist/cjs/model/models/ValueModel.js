...
var import_DefaultValue = require("../logic/DefaultValue.js");
/**
 * @license
...
class ValueModel extends import_Model.Model {
  ...
  get primitiveBase() {
    ...
    if (metabase.metatype === import_definitions.Metatype.enum) {
      return metabase.base;
    }
    if (metabase.metatype === import_definitions.Metatype.bitmap) {
  ...
  get effectiveDefault() {
    return (0, import_DefaultValue.DefaultValue)(this);
  }
  /**
   ...
   */
  get listEntry() {
    return this.member("entry", [import_definitions.ElementTag.Field]);
  }
  /**
   ...
   */
  get allowedBaseTags() {
    if (this.tag === import_definitions.ElementTag.Field) {
      return [import_definitions.ElementTag.Field, import_definitions.ElementTag.Datatype, import_definitions.ElementTag.Attribute];
    }
    return [this.tag, import_definitions.ElementTag.Datatype];
  }
  /**
   * All {@link FieldModel} children}.
   */
  get members() {
    return new import_ModelTraversal.ModelTraversal().findChildren(this, [import_definitions.ElementTag.Field]);
  }
  /**
   * A subset of {@link members} with conflicts resolved by conformance.
   */
  get activeMembers() {
    const cluster = this.owner(import_Model.Model.types[import_definitions.ElementTag.Cluster]);
    return new import_ModelTraversal.ModelTraversal().findActiveMembers(this, false, cluster);
  }
  /**
  ...
  get conformantMembers() {
    const cluster = this.owner(import_Model.Model.types[import_definitions.ElementTag.Cluster]);
    return new import_ModelTraversal.ModelTraversal().findActiveMembers(this, true, cluster);
  }
  /**
  ...
  get fields() {
    ...
    return Object.fromEntries(members.map((member) => [(0, import_String.camelize)(member.name), member]));
  }
  /**
