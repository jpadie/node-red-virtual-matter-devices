...
class ModelTraversal {
  ...
  getTypeName(model) {
    ...
    if (model.tag === import_definitions.ElementTag.Command || model.tag === import_definitions.ElementTag.Event || model.tag === import_definitions.ElementTag.Cluster) {
      return "struct";
    }
  ...
  findShadow(model) {
    ...
    this.operationWithDismissal(model, () => {
      this.visitInheritance(this.findBase(parentOf(model)), (parent) => {
        if (model.id !== void 0 && model.tag !== import_definitions.ElementTag.Command) {
    ...
    return this.operation(() => {
      ...
      const resolve = (field2) => {
        ...
        const referenced = this.findMember(parentOf(model), name, [
          ...
          import_definitions.ElementTag.Field
        ]);
        if (!referenced) {
          return;
        }
    ...
    this.visitInheritance(scope, (model) => {
      ...
      for (const child of model.children) {
        ...
        if (child.id !== void 0) {
          ...
          if (numericLevel !== void 0 && numericLevel < level) {
            continue;
          }
        }
   ...
   */
  findActiveMembers(scope, conformantOnly, cluster) {
    const features = cluster?.featureNames ?? new import_definitions.FeatureSet();
    const supportedFeatures = cluster?.supportedFeatures ?? new import_definitions.FeatureSet();
  ...
  findType(scope, name, tag) {
    ...
    if (memos) {
      const memosForScope2 = memos.types.get(scope);
      if (memosForScope2) {
  ...
  findQualifiedType(scope, path, tag) {
    ...
    function resolve(scope2, position = 0) {
      ...
      if (position === 0 && scope2.tag === import_definitions.ElementTag.Matter && path[0] === "Matter") {
  ...
  findReferences(scope, type) {
    ...
    this.visit(scope, (model) => {
      ...
      if (model.tag === import_definitions.ElementTag.Command && this.findResponse(model) === type) {
        references.push(model);
        return;
      }
      if (model.isType) {
...
function parentOf(model) {
  if (model?.parent === void 0 && model?.tag !== import_definitions.ElementTag.Matter) {
