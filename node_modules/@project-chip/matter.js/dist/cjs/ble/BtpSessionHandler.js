...
class BtpSessionHandler {
  ...
  ackReceiveTimer = import_Time.Time.getTimer(
    "BTP ack timeout",
  ...
  sendAckTimer = import_Time.Time.getTimer(
    "BTP send timeout",
    import_BleConsts.BTP_SEND_ACK_TIMEOUT_MS,
    () => this.btpSendAckTimeoutTriggered()
  );
  isActive = true;
  idleTimeout = import_Time.Time.getTimer("Central Device Idle Timer", import_BleConsts.BTP_CONN_IDLE_TIMEOUT, async () => {
  ...
  static async createFromHandshakeRequest(maxDataSize, handshakeRequestPayload, writeBleCallback, disconnectBleCallback, handleMatterMessagePayload) {
    ...
    logger.debug(`Received BTP handshake request:`, import_Diagnostic.Diagnostic.dict({ maxDataSize, ...handshakeRequest }));
  ...
  static async createAsCentral(handshakeResponsePayload, writeBleCallback, disconnectBleCallback, handleMatterMessagePayload) {
    ...
    logger.debug("Handshake request", import_Diagnostic.Diagnostic.dict(handshakeRequest));
