...
class QrPairingCodeSchema extends import_Schema.Schema {
  encodeInternal(payloadData) {
    if (payloadData.length === 0) throw new import_MatterError.ImplementationError("Provided Payload data is empty");
    return PREFIX + payloadData.map((payloadData2) => {
      const { tlvData } = payloadData2;
      const data = tlvData !== void 0 && tlvData.length > 0 ? import_ByteArray.ByteArray.concat(QrCodeDataSchema.encode(payloadData2), tlvData) : QrCodeDataSchema.encode(payloadData2);
      return import_Base38Schema.Base38.encode(data);
    }).join("*");
  }
  decodeInternal(encoded) {
    if (!encoded.startsWith(PREFIX)) throw new import_MatterError.UnexpectedDataError("The pairing code should start with MT:");
  ...
  decodeTlvData(data, schema = (0, import_TlvObject.TlvObject)(QrCodeTlvDataDefaultFields)) {
    ...
    if (decoded.serialNumber !== void 0) {
      if (!Array.isArray(decoded.serialNumber) || decoded.serialNumber.length !== 1 || decoded.serialNumber[0].typeLength === void 0 || decoded.serialNumber[0].value === void 0) {
        throw new import_MatterError.UnexpectedDataError("Invalid serial number TLV data");
      }
  ...
  encodeTlvData(data, schema = (0, import_TlvObject.TlvObject)(QrCodeTlvDataDefaultFields)) {
    const dataToEncode = (0, import_DeepCopy.deepCopy)(data);
    if ("serialNumber" in dataToEncode && dataToEncode.serialNumber !== void 0) {
      switch (typeof dataToEncode.serialNumber) {
        ...
        case "number":
          ...
          break;
        default:
          throw new import_MatterError.ImplementationError("Invalid serial number data");
      }
    }
...
class ManualPairingCodeSchema extends import_Schema.Schema {
  encodeInternal({ discriminator, passcode, vendorId, productId }) {
    if (discriminator === void 0) throw new import_MatterError.UnexpectedDataError("discriminator is required");
    if (discriminator > 4095) throw new import_MatterError.UnexpectedDataError("discriminator value must be less than 4096");
    let result = "";
