  ...
  responseList.forEach((values) => {
    ...
    try {
      ...
      if (eventDetail === void 0) {
        logger.debug(
          `Decode unknown event ${import_Diagnostic.Diagnostic.hex(clusterId)}/${import_Diagnostic.Diagnostic.hex(eventId)} via the AnySchema.`
        );
        const eventName = `Unknown (${import_Diagnostic.Diagnostic.hex(eventId)})`;
