...
var import_ManufacturerExtensibleIdentifier = require("../../datatype/ManufacturerExtensibleIdentifier.js");
var import_model = require("../../model/index.js");
var import_AttributeList = require("../../model/standard/elements/AttributeList.js");
var import_EventList = require("../../model/standard/elements/EventList.js");
var import_elements = require("../../model/standard/elements/index.js");
 ...
 */
const GLOBAL_COMMANDLIST_IDS = [import_elements.GeneratedCommandList.id, import_elements.AcceptedCommandList.id];
const GLOBAL_ATTRIBUTES_LIST = [...GLOBAL_COMMANDLIST_IDS, import_EventList.EventList.id, import_AttributeList.AttributeList.id];
...
class InteractionEndpointStructure {
  ...
  verifyAndInitializeStructureElementsFromEndpoint(endpoint) {
    if (endpoint.number === void 0) {
      throw new import_MatterError.InternalError(`Endpoint ID is undefined. It needs to be initialized first!`);
    }
    ...
    if (this.endpoints.has(endpoint.number))
      throw new import_MatterError.ImplementationError(`Endpoint ID ${endpoint.number} exists twice`);
  ...
  validateConcreteAttributePath(endpointId, clusterId, attributeId) {
    if (!this.hasEndpoint(endpointId)) {
      throw new import_StatusCode.StatusResponseError(`Endpoint ${endpointId} does not exist.`, import_StatusCode.StatusCode.UnsupportedEndpoint);
    }
    if (!this.hasClusterServer(endpointId, clusterId)) {
      throw new import_StatusCode.StatusResponseError(`Cluster ${clusterId} does not exist.`, import_StatusCode.StatusCode.UnsupportedCluster);
    }
    if (this.hasAttribute(endpointId, clusterId, attributeId)) return true;
    throw new import_StatusCode.StatusResponseError(`Attribute ${attributeId} does not exist`, import_StatusCode.StatusCode.UnsupportedAttribute);
  }
  ...
  validateConcreteEventPath(endpointId, clusterId, eventId) {
    ...
    throw new import_StatusCode.StatusResponseError(`Event ${eventId} does not exist`, import_StatusCode.StatusCode.UnsupportedEvent);
  }
  ...
  validateConcreteCommandPath(endpointId, clusterId, commandId) {
    ...
    throw new import_StatusCode.StatusResponseError(`Command ${commandId} does not exist`, import_StatusCode.StatusCode.UnsupportedCommand);
  }
  ...
  attributePathMatchesWildcardPathFlags({ endpointId, clusterId, attributeId }, wildcardPathFlags) {
    ...
    if (attributeId >= 65528 && attributeId <= 65531) {
      ...
      if (wildcardPathFlags.skipAttributeList && attributeId === import_AttributeList.AttributeList.id) {
        return true;
      }
      if (wildcardPathFlags.skipEventList && attributeId === import_EventList.EventList.id) {
        return true;
      }
    ...
    if (wildcardPathFlags.skipCustomElements && (import_ManufacturerExtensibleIdentifier.Mei.hasCustomMeiPrefix(clusterId) || import_ManufacturerExtensibleIdentifier.Mei.hasCustomMeiPrefix(attributeId))) {
      return true;
    }
  ...
  validateAnyPathDataTypes(data) {
    ...
    if (endpointId !== void 0) {
      (0, import_EndpointNumber.EndpointNumber)(endpointId);
    }
    if (clusterId !== void 0) {
      (0, import_ClusterId.ClusterId)(clusterId);
    }
    if (attributeId !== void 0) {
      (0, import_AttributeId.AttributeId)(attributeId);
    }
    if (eventId !== void 0) {
      (0, import_EventId.EventId)(eventId);
    }
    if (commandId !== void 0) {
      (0, import_CommandId.CommandId)(commandId);
    }
  }
}
