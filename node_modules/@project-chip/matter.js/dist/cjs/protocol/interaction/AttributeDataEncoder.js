...
function encodeAttributePayload(attributePayload, options) {
  ...
  return import_InteractionProtocol.TlvAttributeReport.encodeTlv({
...
function chunkAttributePayload(attributePayload) {
  ...
  if (attributeData === void 0) {
    throw new import_MatterError.MatterFlowError(
      `Cannot chunk an AttributePayload with just a attributeStatus: ${import_Logger.Logger.toJSON(attributePayload)}`
    );
  }
  ...
  if (!(schema instanceof import_TlvArray.ArraySchema) || !Array.isArray(payload)) {
    throw new import_MatterError.MatterFlowError(
      `Cannot chunk an AttributePayload with attributeData that is not an array: ${import_Logger.Logger.toJSON(
        attributePayload
      )}`
    );
  }
