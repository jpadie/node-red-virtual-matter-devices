 ...
 */
const logger = import_Logger.Logger.get("AccessControlManager");
const ImplicitDefaultPaseAclEntry = {
  fabricIndex: import_FabricIndex.FabricIndex.NO_FABRIC,
  // not fabric-specific
  privilege: import_Cluster.AccessLevel.Administer,
  authMode: import_AccessControlCluster.AccessControl.AccessControlEntryAuthMode.Pase,
  subjects: [],
  targets: []
  // entire node
};
...
class AccessDeniedError extends import_StatusCode.StatusResponseError {
  constructor(message) {
    super(message ?? "Unauthorized", import_StatusCode.StatusCode.UnsupportedAccess);
  }
}
class AccessControlManager {
  ...
  #subjectMatches(aclSubject, isdSubject) {
    ...
    if (!import_NodeId.NodeId.isCaseAuthenticatedTag(aclSubject) || !import_NodeId.NodeId.isCaseAuthenticatedTag(isdSubject)) {
      return false;
    }
    const aclSubjectCat = import_NodeId.NodeId.extractAsCaseAuthenticatedTag(aclSubject);
    const isdSubjectCat = import_NodeId.NodeId.extractAsCaseAuthenticatedTag(isdSubject);
    return import_CaseAuthenticatedTag.CaseAuthenticatedTag.getIdentifyValue(aclSubjectCat) === import_CaseAuthenticatedTag.CaseAuthenticatedTag.getIdentifyValue(isdSubjectCat) && import_CaseAuthenticatedTag.CaseAuthenticatedTag.getVersion(isdSubjectCat) >= import_CaseAuthenticatedTag.CaseAuthenticatedTag.getVersion(aclSubjectCat);
  }
  /**
  ...
  #addGrantedPrivilege(grantedPrivileges, privilege) {
    ...
    switch (privilege) {
      case import_Cluster.AccessLevel.ProxyView:
        grantedPrivileges.add(import_Cluster.AccessLevel.View);
        break;
      case import_Cluster.AccessLevel.Operate:
        grantedPrivileges.add(import_Cluster.AccessLevel.View);
        break;
      case import_Cluster.AccessLevel.Manage:
        grantedPrivileges.add(import_Cluster.AccessLevel.Operate);
        grantedPrivileges.add(import_Cluster.AccessLevel.View);
        break;
      case import_Cluster.AccessLevel.Administer:
        grantedPrivileges.add(import_Cluster.AccessLevel.Manage);
        grantedPrivileges.add(import_Cluster.AccessLevel.Operate);
        grantedPrivileges.add(import_Cluster.AccessLevel.ProxyView);
        grantedPrivileges.add(import_Cluster.AccessLevel.View);
        break;
    }
  }
  ...
  getGrantedPrivileges(session, endpoint, clusterId) {
    ...
    if (subjectDesc.authMode === import_AccessControlCluster.AccessControl.AccessControlEntryAuthMode.Pase && subjectDesc.isCommissioning) {
      this.#addGrantedPrivilege(grantedPrivileges, import_Cluster.AccessLevel.Administer);
    }
    for (const aclEntry of acl) {
      if (grantedPrivileges.has(import_Cluster.AccessLevel.Administer)) {
        break;
      }
      if (aclEntry.fabricIndex === import_FabricIndex.FabricIndex.NO_FABRIC || aclEntry.fabricIndex !== subjectDesc.fabricIndex) {
        logger.debug(
      ...
      if (aclEntry.subjects === null || aclEntry.subjects.length === 0) {
        if (aclEntry.authMode !== import_AccessControlCluster.AccessControl.AccessControlEntryAuthMode.Case && aclEntry.authMode !== import_AccessControlCluster.AccessControl.AccessControlEntryAuthMode.Group) {
          throw new import_MatterError.MatterFlowError("ACL error: only CASE and Group auth can have empty subjects");
        }
      } else {
      ...
      if (aclEntry.targets === null || aclEntry.targets.length === 0) {
        ...
        for (const {
          ...
          if (targetClusterId === null && targetEndpointId === null && targetDeviceType === null) {
            throw new import_MatterError.MatterFlowError("ACL error: target cannot be empty");
          }
          if (targetEndpointId !== null && targetDeviceType !== null) {
            throw new import_MatterError.MatterFlowError("ACL error: target cannot specify both endpoint and device type");
          }
    ...
    if (subjectDesc.authMode === import_AccessControlCluster.AccessControl.AccessControlEntryAuthMode.Group && grantedPrivileges.has(import_Cluster.AccessLevel.Administer)) {
      throw new import_MatterError.MatterFlowError("ACL error: should never grant Administer privilege to a Group");
    }
  ...
  #getIsdFromMessage(session) {
    ...
    if (session.isPase) {
      isd.authMode = import_AccessControlCluster.AccessControl.AccessControlEntryAuthMode.Pase;
      isd.isCommissioning = true;
      isd.subjects.push((0, import_NodeId.NodeId)(0));
      if (fabric) {
        isd.fabricIndex = fabric.fabricIndex;
      }
    } else {
      isd.authMode = import_AccessControlCluster.AccessControl.AccessControlEntryAuthMode.Case;
      isd.subjects.push(session.peerNodeId);
      session.caseAuthenticatedTags.forEach((cat) => isd.subjects.push(import_NodeId.NodeId.fromCaseAuthenticatedTag(cat)));
      if (fabric === void 0) {
        throw new import_MatterError.MatterFlowError("ACL error: fabric is undefined");
      }
      isd.fabricIndex = fabric.fabricIndex;
    }
    return isd;
