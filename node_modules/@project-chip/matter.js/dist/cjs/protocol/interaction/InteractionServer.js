...
const INTERACTION_MODEL_REVISION = import_Specification.Specification.INTERACTION_MODEL_REVISION;
...
class InteractionServer {
  ...
  constructor({
    ...
    this.#subscriptionConfig = import_SubscriptionOptions.SubscriptionOptions.configurationFor(subscriptionOptions);
      ...
      if (attributes.length === 0) {
        if (isConcreteAttributePath(requestPath)) {
          ...
          try {
            ...
            import_StatusCode.StatusResponseError.accept(e);
            logger.debug(
          ...
          if (versionFilterValue !== void 0 && versionFilterValue === version) {
            logger.debug(
              `Read attribute from ${exchange.channel.name}: ${this.#endpointStructure.resolveAttributeName(
                path
              )}=${import_Logger.Logger.toJSON(value)} (version=${version}) ignored because of dataVersionFilter`
            );
            continue;
          }
          logger.debug(
      ...
      for (const requestPath of eventRequests) {
        ...
        eventReportsPayload.push(
          ...reportsForPath.sort((a, b) => {
            const eventNumberA = a.eventData?.eventNumber ?? (0, import_EventNumber.EventNumber)(0);
            const eventNumberB = b.eventData?.eventNumber ?? (0, import_EventNumber.EventNumber)(0);
    ...
    for (const writeRequest of writeData) {
      ...
      if (attributes.length === 1 && isConcreteAttributePath(writePath)) {
        ...
        if (attribute.requiresTimedInteraction && !receivedWithinTimedInteraction) {
          ...
          writeResults.push({ path: writePath, statusCode: import_StatusCode.StatusCode.NeedsTimedInteraction });
          continue;
        }
        if (attribute instanceof import_AttributeServer.FabricScopedAttributeServer && (!exchange.session.isSecure || !exchange.session.fabric)) {
          ...
          writeResults.push({ path: writePath, statusCode: import_StatusCode.StatusCode.UnsupportedAccess });
          continue;
        }
        if (dataVersion !== void 0) {
          ...
          if (currentDataVersion !== void 0) {
            if (dataVersion !== currentDataVersion) {
              ...
              writeResults.push({ path: writePath, statusCode: import_StatusCode.StatusCode.DataVersionMismatch });
              continue;
            }
      ...
      for (const { path, attribute } of attributes) {
        ...
        try {
          ...
          logger.debug(
            `Handle write request from ${exchange.channel.name} resolved to: ${this.#endpointStructure.resolveAttributeName(path)}=${import_Logger.Logger.toJSON(
              value
          ...
          await this.writeAttribute(
            ...
            schema instanceof import_TlvArray.ArraySchema
          );
          if (schema instanceof import_TlvArray.ArraySchema && !attributeListWrites.has(attribute)) {
            attributeListWrites.add(attribute);
          }
        } catch (error) {
          if (import_StatusCode.StatusResponseError.is(error, import_StatusCode.StatusCode.UnsupportedAccess)) {
          ...
          if (attributes.length === 1 && isConcreteAttributePath(writePath)) {
            logger.error(
              ...
              error instanceof import_StatusCode.StatusResponseError ? error.message : error
            );
            ...
            writeResults.push({ path, statusCode: import_StatusCode.StatusCode.ConstraintError });
            continue;
          } else {
            logger.debug(
        ...
        writeResults.push({ path, statusCode: import_StatusCode.StatusCode.Success });
      }
    }
  ...
  async handleSubscribeRequest(exchange, {
    ...
    logger.info(
      `Successfully created subscription ${subscriptionId} for Session ${session.id}. Updates: ${minIntervalFloorSeconds} - ${maxIntervalCeilingSeconds} => ${maxInterval} seconds (sendInterval = ${subscriptionHandler.sendInterval} seconds)`
    );
  ...
  async handleInvokeRequest(exchange, { invokeRequests, timedRequest, suppressResponse, interactionModelRevision }, messenger, message) {
    ...
    const emptyInvokeResponseBytes = import_InteractionProtocol.TlvInvokeResponseForSend.encode(invokeResponseMessage);
    ...
    const processResponseResult = async (invokeResponse) => {
      ...
      const encodedInvokeResponse = import_InteractionProtocol.TlvInvokeResponseData.encodeTlv(invokeResponse);
      const invokeResponseBytes = import_TlvAny.TlvAny.getEncodedByteLength(encodedInvokeResponse);
      if (messageSize + invokeResponseBytes > exchange.maxPayloadSize || invokeResultsProcessed === invokeRequests.length) {
        ...
        if (invokeResponseMessage.invokeResponses.length > 0) {
          ...
          await messenger.send(
            ...
            import_InteractionProtocol.TlvInvokeResponseForSend.encode({
    ...
    for (const { commandPath, commandFields, commandRef } of invokeRequests) {
      ...
      for (const { command, path } of commands) {
        ...
        if (endpointId === void 0) {
          ...
          if (isConcretePath) {
            await processResponseResult({
              status: {
                commandPath: path,
                status: { status: import_StatusCode.StatusCode.UnsupportedEndpoint },
                commandRef
              }
            });
          }
        ...
        if (command.requiresTimedInteraction && !receivedWithinTimedInteraction) {
          ...
          if (isConcretePath) {
            await processResponseResult({
              status: {
                commandPath: path,
                status: { status: import_StatusCode.StatusCode.NeedsTimedInteraction },
                commandRef
              }
            });
          }
        ...
        if (getMatterModelClusterCommand(clusterId, commandId)?.fabricScoped && (!exchange.session.isSecure || !exchange.session.fabric)) {
          ...
          if (isConcretePath) {
            await processResponseResult({
              status: { commandPath: path, status: { status: import_StatusCode.StatusCode.UnsupportedAccess }, commandRef }
            });
          }
          continue;
        }
        ...
        try {
          ...
          const errorLogText = `Error ${import_Diagnostic.Diagnostic.hex(errorCode)}${e.clusterCode !== void 0 ? `/${import_Diagnostic.Diagnostic.hex(e.clusterCode)}` : ""} while invoking command: ${e.message}`;
          if (e instanceof import_ValidationError.ValidationError) {
            logger.info(
              `Validation-${errorLogText}${e.fieldName !== void 0 ? ` in field ${e.fieldName}` : ""}`
            );
            if (errorCode === import_StatusCode.StatusCode.InvalidAction) {
              errorCode = import_StatusCode.StatusCode.InvalidCommand;
            }
          } else {
